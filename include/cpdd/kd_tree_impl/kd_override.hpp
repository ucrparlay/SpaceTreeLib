...
void KdTree<Point, SplitRule, kBDO>::KNN(Node* T, const Point& q, const DimsType DIM,
...
template<typename Point, typename SplitRule, uint8_t kBDO>
template<typename Range>
size_t KdTree<Point, SplitRule, kBDO>::RangeQuery(const Box& query_box, Range&& Out) {
    size_t s = 0;
