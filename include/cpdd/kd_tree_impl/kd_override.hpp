...
template<typename Point, typename SplitRule>
size_t KdTree<Point, SplitRule>::RangeCount(const Box& bx) {
...
template<typename Point, typename SplitRule>
size_t KdTree<Point, SplitRule>::RangeCount(const Circle& cl) {
...
template<typename Point, typename SplitRule>
template<typename Range>
size_t KdTree<Point, SplitRule>::RangeQuery(const Box& query_box, Range&& Out) {
    size_t s = 0;
