...
typename octTree<Point>::ZValueType octTree<Point>::interleave_bits(
    ...
    for (ZBitType i = 0; i < KEY_BITS / DIM; i++) {
        for (DimsType d = 0; d < DIM; d++) {
            id = id |
                 (((p->pnt[d] >> i) & static_cast<ZValueType>(1)) << (loc++));
        }
    }
    return id;
}
...
Node* octTree<Point>::build_recursive_with_z_value_pointer(
    ...
    parlay::par_do_if(
        ...
        [&] {
            L = build_recursive_with_z_value_pointer(In.cut(0, pos), bit - 1,
                                                     DIM);
        },
        [&] {
            R = build_recursive_with_z_value_pointer(In.cut(pos, n), bit - 1,
                                                     DIM);
        });
...
void octTree<Point>::build_z_value(slice A, const DimsType DIM) {
    ...
    this->root_ = build_recursive_with_z_value(parlay::make_slice(z_value_arr),
...
template<typename Point>
Node* octTree<Point>::build_recursive_with_z_value(ZValueSlice In, ZBitType bit,
                                                   const DimsType DIM) {
    size_t n = In.size();
...
void octTree<Point>::build_point_z_value(slice A, const DimsType DIM) {
    ...
    this->root_ = build_recursive_point_z_value(parlay::make_slice(z_value_arr),
                                                DIM * (KEY_BITS / DIM), DIM);
