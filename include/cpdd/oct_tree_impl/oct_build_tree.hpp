...
/**/
/*template<typename Point>*/
/*typename octTree<Point>::ZValueType octTree<Point>::interleave_bits(*/
/*    Point* p, const DimsType DIM) {*/
/*    ZBitType loc = 0;*/
/*    ZValueType id = 0;*/
/*    for (ZBitType i = 0; i < KEY_BITS / DIM; i++) {*/
/*        for (DimsType d = 0; d < DIM; d++) {*/
/*            id = id |*/
/*                 (((p->pnt[d] >> i) & static_cast<ZValueType>(1)) <<
 * (loc++));*/
/*        }*/
/*    }*/
/*    return id;*/
...
/**/
/*template<typename Point>*/
/*typename octTree<Point>::ZValueType octTree<Point>::get_z_value(*/
/*    const Point& p) {*/
/*    if constexpr (std::tuple_size<typename Point::Coords>::value == 2) {*/
/*        return libmorton::m2D_e_BMI<ZValueType, Coord>(p.pnt[0], p.pnt[1]);*/
/*    } else {*/
/*        return libmorton::m3D_e_BMI<ZValueType, Coord>(p.pnt[0], p.pnt[1],*/
/*                                                       p.pnt[2]);*/
/*    }*/
/*}*/
/**/
...
/**/
/*    this->root_ = build_recursive_with_z_value_pointer(*/
...
/*    ZValuePointerSlice In, ZBitType bit, const DimsType DIM) {*/
/*    size_t n = In.size();*/
/**/
/*    if (n <= BaseTree::kLeaveWrap) {*/
/*        return allocLeafNode<Point, ZValuePointerSlice, AllocNormalLeafTag,*/
/*                             parlay::uninitialized_relocate_tag>(*/
/*            In, std::max(In.size(),
 * static_cast<size_t>(BaseTree::kLeaveWrap)));*/
/*    }*/
/**/
/*    if (bit == 0) {*/
/*        return allocLeafNode<Point, ZValuePointerSlice, alloc_fat_leaf_tag,*/
/*                             parlay::uninitialized_relocate_tag>(In,
 * In.size());*/
/*    }*/
/**/
/*    ZValueType val = (static_cast<ZValueType>(1)) << (bit - 1);*/
/*    ZValueType mask = (bit == 64) ? ~(static_cast<ZValueType>(0))*/
/*                                  : ~(~(static_cast<ZValueType>(0)) << bit);*/
/*    auto less = [&](const ZValuePointerPair& x) {*/
...
/*    Node *L, *R;*/
/*    parlay::par_do_if(*/
/*        n >= this->kSerialBuildCutoff,*/
/*        [&] {*/
/*            L = build_recursive_with_z_value_pointer(In.cut(0, pos), bit -
 * 1,*/
/*                                                     DIM);*/
/*        },*/
/*        [&] {*/
/*            R = build_recursive_with_z_value_pointer(In.cut(pos, n), bit -
 * 1,*/
/*                                                     DIM);*/
/*        });*/
...
/*    parlay::internal::integer_sort_inplace(*/
/*        parlay::make_slice(z_value_arr),*/
/*        [&](const ZValueType& val) { return val; });*/
/*    t.next("integer_sort_inplace");*/
/**/
/*    this->root_ =
 * build_recursive_with_z_value(parlay::make_slice(z_value_arr),*/
...
/**/
/*template<typename Point>*/
/*Node* octTree<Point>::build_recursive_with_z_value(ZValueSlice In, ZBitType
 * bit,*/
/*                                                   const DimsType DIM) {*/
/*    size_t n = In.size();*/
/*    if (bit == 0 || n <= BaseTree::kLeaveWrap) {*/
/*        // BUG:: need to check the type of the leaf*/
/*        return AllocLeafNode<ZValueType, ZValueSlice, AllocNormalLeafTag,
 * 32,*/
...
/**/
/*    this->root_ =
 * build_recursive_point_z_value(parlay::make_slice(z_value_arr),*/
/*                                                DIM * (KEY_BITS / DIM),
 * DIM);*/
...
/**/
/*template<typename Point>*/
/*Node* octTree<Point>::build_recursive_point_z_value(ZValuePointSlice In,*/
/*                                                    ZBitType bit,*/
/*                                                    const DimsType DIM) {*/
/*    size_t n = In.size();*/
/*    if (bit == 0 || n <= BaseTree::kLeaveWrap) {*/
/*        return allocLeafNode<Point, ZValuePointSlice>(*/
/*            In, std::max(In.size(),
 * static_cast<size_t>(BaseTree::kLeaveWrap)));*/
/*    }*/
/**/
...
/*    ZValueType mask = (bit == 64) ? ~(static_cast<ZValueType>(0))*/
/*                                  : ~(~(static_cast<ZValueType>(0)) << bit);*/
/*    auto less = [&](const ZValuePointPair& x) {*/
/*        return (x.first & mask) < val;*/
/*    };*/
/*    size_t pos = parlay::internal::binary_search(In, less);*/
/**/
...
/*    t.next("integer_sort_inplace");*/
/**/
/*    this->root_ = build_recursive_point(A, DIM * sizeof(Coord) * 8 - 1,
 * DIM);*/
/*    t.next("build_recursive");*/
/*    assert(this->root_ != nullptr);*/
/*    return;*/
...
/**/
/*template<typename Point>*/
/*Node* octTree<Point>::build_recursive_point(slice In, ZBitType bit,*/
/*                                            const DimsType DIM) {*/
/*    size_t n = In.size();*/
/*    if (bit == 0 || n <= BaseTree::kLeaveWrap) {*/
/*        return allocLeafNode<Point, slice>(*/
/*            In, std::max(In.size(),
 * static_cast<size_t>(BaseTree::kLeaveWrap)));*/
/*    }*/
/**/
...
/*    });*/
/*    size_t pos = std::lower_bound(bits.begin(), bits.end(), 1) -
 * bits.begin();*/
/**/
/*    if (pos == 0 || pos == n) {*/
