...
struct leaf : node {
  ...
  leaf(slice In, const size_t size, assign_func const& ASSIGN_FUNC) :
...
template<typename point, typename slice, typename assign_func>
static leaf<point, slice, assign_func>* alloc_leaf_node(
    slice In, const uint_fast8_t alloc_size,
  ...
  new (o) leaf(In, alloc_size, ASSIGN_FUNC);
