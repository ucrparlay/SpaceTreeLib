...
template<typename Point, typename SplitRule, uint_fast8_t kMD,
         uint_fast8_t kBDO>
auto OrthTree<Point, SplitRule, kMD, kBDO>::RangeCount(const Box& bx) {
...
template<typename Point, typename SplitRule, uint_fast8_t kMD,
         uint_fast8_t kBDO>
auto OrthTree<Point, SplitRule, kMD, kBDO>::RangeCount(const Circle& cl) {
...
template<typename Point, typename SplitRule, uint_fast8_t kMD,
         uint_fast8_t kBDO>
template<typename Range>
auto OrthTree<Point, SplitRule, kMD, kBDO>::RangeQuery(const Box& query_box,
                                                       Range&& Out) {
    RangeQueryLogger logger;
    size_t s = 0;
