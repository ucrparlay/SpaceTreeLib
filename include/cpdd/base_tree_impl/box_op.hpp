...
inline bool BaseTree<Point>::LegalBox(const Box& bx) {
...
template<typename Point>
inline bool BaseTree<Point>::WithinBox(const Box& a, const Box& b) {
    assert(LegalBox(a));
    assert(LegalBox(b));
...
template<typename Point>
inline bool BaseTree<Point>::WithinBox(const Point& p, const Box& bx) {
    assert(LegalBox(bx));
...
template<typename Point>
inline bool BaseTree<Point>::BoxIntersectBox(const Box& a, const Box& b) {
...
template<typename Point>
inline typename BaseTree<Point>::Box BaseTree<Point>::GetEmptyBox() {
    return Box(Point(std::numeric_limits<Coord>::max()), Point(std::numeric_limits<Coord>::min()));
}

template<typename Point>
typename BaseTree<Point>::Box BaseTree<Point>::GetBox(const Box& x, const Box& y) {
...
template<typename Point>
inline bool BaseTree<Point>::WithinCircle(const Box& bx, const Circle& cl) {
...
template<typename Point>
inline bool BaseTree<Point>::WithinCircle(const Point& p, const Circle& cl) {
    Coord r = 0;
    for (DimsType i = 0; i < cl.first.get_dim(); i++) {
...
template<typename Point>
inline bool BaseTree<Point>::CircleIntersectBox(const Circle& cl, const Box& bx) {
