...
void BaseTree<point>::delete_tree_recursive(node* T, bool granularity) {
    ...
    if (T->is_leaf) {
        ...
        // delete in parallel
        parlay::par_do_if((granularity && T->size > SERIAL_BUILD_CUTOFF) || (!granularity && TI->aug),
                          [&] { delete_tree_recursive<leaf_node_type, interior_node_type>(TI->left, granularity); },
                          [&] { delete_tree_recursive<leaf_node_type, interior_node_type>(TI->right, granularity); });
