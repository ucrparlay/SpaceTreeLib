...
void QuadTree<Point, SplitRule, kBDO>::KNN(Node* T, const Point& q,
                                         const DimsType DIM,
...
template<typename Point, typename SplitRule, uint8_t kBDO>
template<typename Range>
void QuadTree<Point, SplitRule, kBDO>::Flatten(Range&& Out) {
    BT::template FlattenRec<Leaf, Interior>(this->root_,
                                            parlay::make_slice(Out), false);
}

template<typename Point, typename SplitRule, uint8_t kBDO>
size_t QuadTree<Point, SplitRule, kBDO>::RangeCount(const Box& bx) {
...
template<typename Point, typename SplitRule, uint8_t kBDO>
size_t QuadTree<Point, SplitRule, kBDO>::RangeCount(const Circle& cl) {
...
template<typename Point, typename SplitRule, uint8_t kBDO>
template<typename Range>
size_t QuadTree<Point, SplitRule, kBDO>::RangeQuery(const Box& query_box,
                                                  Range&& Out) {
    size_t s = 0;
...
template<typename Point, typename SplitRule, uint8_t kBDO>
void QuadTree<Point, SplitRule, kBDO>::DeleteTree() {
