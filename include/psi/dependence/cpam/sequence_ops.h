...
struct sequence_ops : Tree {
	...
	static regular_node *to_tree_impl(ET *A, uint32_t n, uint32_t depth = 0)
	{
		...
		if (n == B && depth > 0) {
			return (regular_node
					*)(Tree::make_single_compressed_node(
				A, n));
		}
	...
	static expose_simple_tuple expose_simple(node *p)
	{
		if (Tree::is_regular(p)) {
			...
			return expose_simple_tuple(rp->lc, Tree::get_entry(p),
						   rp->rc);
		} else {
			std::cout << "error in expose_simple: applied to a "
				     "compressed node"
				  << std::endl;
			return expose_simple_tuple(nullptr, Tree::get_entry(p),
						   nullptr);
		}
	}
	...
	static expose_tuple expose(ptr a)
	{
		...
		if (is_regular) {
			...
			auto ret_tup = expose_tuple(ptr(rp->lc, copy),
						    Tree::get_entry(rp),
						    ptr(rp->rc, copy), ret);
			return ret_tup;
		} else {
			// convert to tree
			// if (print) {
			//  std::cout << "Hit a compressed node, converting to
			//  tree. Size = " << Tree::size(p) << std::endl; size_t
			//  sz = 0; auto fn = [&] (const auto& et) {
			//    std::cout << std::get<0>(et) << " " << sz <<
			//    std::endl; sz++;
			//  };
			...
			// }
			return expose_tuple(ptr(root->lc, false),
					    Tree::get_entry(root),
					    ptr(root->rc, false), root);
		}
	}

	static void reorder(node *c)
	{
		Tree::reorder(c);
	}
	...
	static std::optional<ET> select(node *a, size_t rank)
	{
		...
		if (Tree::is_compressed(a))
			return select_compressed(a, rank);
	...
	static std::tuple<node *, size_t, size_t>
	subseq_root(node *b, size_t left, size_t right)
	{
		if (!b)
		...
		if (right < left_size) {
			...
			return subseq_root(rb->rc, left - left_size - 1,
					   right - left_size - 1);
		}
		return {b, left, right};
	}
	...
	static node *suffix(node *a, size_t const rank)
	{
		if (!a)
			return NULL;
		if (Tree::is_compressed(a)) {
		...
		if (rank > left_size + 1)
			return suffix(ar->rc, rank - left_size - 1);
		regular_node *join =
			Tree::make_regular_node(Tree::get_entry(ar));
		node *l = suffix(ar->lc, rank);
		GC::increment(ar->rc);
	...
	static node *subseq(node *b, size_t low, size_t high)
	{
		...
		if (Tree::is_compressed(r)) {
			...
			return join(suffix(lc, low), e,
				    take(rc, high - Tree::size(lc) - 1),
				    nullptr);
		}
	}
	...
	static node *join2_i(ptr b1, ptr b2)
	{
		...
		if (b1.size() > b2.size()) {
			if (b1.is_compressed() ||
			    Tree::will_be_compressed(b1.size() + b2.size())) {
				auto ret = Tree::make_compressed(b1.node_ptr(),
								 b2.node_ptr());
			...
			if (b2.is_compressed() ||
			    Tree::will_be_compressed(b1.size() + b2.size())) {
				return Tree::make_compressed(b1.node_ptr(),
							     b2.node_ptr());
			}
	...
	static node *map_compressed(typename InTree::ptr b, Func const &f)
	{
	...
	static node *map(typename InTree::ptr b, Func const &f)
	{
		...
		auto P = utils::fork<node *>(
			true, [&]() { return map<InTree>(std::move(lc), f); },
	...
	// nodes, total size of leaf nodes).
	...
	template <class F>
	static bool is_sorted(node *a, F const &less)
	{
						   ...
						   bool par = false)
	{
		if (a == nullptr)
			return {};
		if (Tree::is_compressed(a))
			return find_unsorted_compressed(a, e);
		auto an = (regular_node *)a;

		auto P = utils::fork<std::optional<size_t>>(
			par, [&]() { return find_unsorted(an->lc, e, par); },
				  ...
				  size_t granularity = kNodeLimit)
	{
		if (a.empty())
			return;
		if (a.is_compressed()) {
			auto c = a.unsafe_ptr(); // Why unsafe ptr here??
		...
		utils::fork_no_result(
			lsize >= granularity,
			[&]() {
				foreach_index(std::move(lc), start, f,
					      granularity);
			},
			[&]() {
				    ...
				    size_t granularity = kNodeLimit)
	{
		...
		utils::fork_no_result(
			true, [&]() { foreach_index_2(an->lc, f); },
			[&]() { foreach_index_2(an->rc, f); });
	}

	// F : entry x index -> bool
	template <typename F>
	static bool foreach_cond(node *a, F const &f)
	{
		if (a == nullptr)
			return true;
		if (Tree::is_compressed(a)) {
	...
	static bool foreach_cond_par(node *a, F const &f, C const &cond,
				     bool par = false)
	{
		if (a == nullptr)
			return true;
	...
	template <typename F>
	static void foreach_seq(ptr a, F const &f)
	{
		if (a.empty())
			return;
		auto [lc, e, rc, root] = expose(std::move(a));
	...
	//  template<class R, class F>
	//  static typename R::T semi_map_reduce(node* b, F& f, R reduce, size_t
	//  grain) {
	//    using T = typename R::T;
