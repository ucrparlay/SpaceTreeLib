  ...
  while (bits > 0) {
    ...
    if (swapped) {
      ...
      seq_count_sort_<uninitialized_relocate_tag>(
          Out, In, delayed_seq<size_t>(n, get_key), counts, num_buckets);
    }
      ...
      seq_count_sort_<uninitialized_relocate_tag>(
          In, Out, delayed_seq<size_t>(n, get_key), counts, num_buckets);
    }
...
void seq_radix_sort(slice<InIterator, InIterator> In,
  ...
  if constexpr (inplace_tag::value == true) {
    ...
    if (odd) {
      ...
      if constexpr (std::is_same_v<assignment_tag,
                                   uninitialized_relocate_tag>) {
        parlay::uninitialized_relocate(In.begin(), In.end(), Tmp.begin());
      } else {
        for (size_t i = 0; i < n; i++) assign_uninitialized(Tmp[i], In[i]);
      }
      ...
      if constexpr (std::is_same_v<assignment_tag,
        ...
        for (size_t i = 0; i < n; i++) assign_uninitialized(Out[i], In[i]);
      }
...
sequence<size_t> integer_sort_r(slice<InIterator, InIterator> In,
  ...
  //  - If inplace_tag is std::true_type, then In points to mutable (non-const)
  //  memory
  //  - If assignment_tag is uninitialized_copy_tag, then In points to objects
  //  of a type
  ...
  //  memory
  //  - If inplace_tag is std::false_type, then Out points to the resulting
  //  sorted objects
  //
  // Parameter Tmp
  //  [Preconditions]
  //  - If inplace_tag is std::true_type, then Tmp points to the same range as
  //  In
  //  - If inplace_tag is std::false_type and assignment_tag is
  //  uninitialized_copy_tag,
  ...
  static_assert(
      std::is_same_v<range_value_type_t<slice<InIterator, InIterator>>,
  ...
  // assignment_tag is only allowed to be uninitialized_copy_tag if inplace_tag
  // is std::false type
  static_assert(inplace_tag::value == false ||
                std::is_same_v<assignment_tag, uninitialized_relocate_tag>);
  ...
  if (key_bits == 0) {
    ...
    if constexpr (inplace_tag::value == false) {
      ...
      // this would not allow us to take advantage of the possibly more
      // efficient uninitialized_relocate_n, which can memcpy multiple elements
      // at a time to save on performing every copy individually.
      if constexpr (std::is_same_v<assignment_tag,
                                   uninitialized_relocate_tag>) {
        parlay::uninitialized_relocate(In.begin(), In.end(), Out.begin());
      } else {
  ...
  else if ((n < PARLAY_INTEGER_SORT_BASE_CASE_SIZE || parallelism < .0001) &&
           !return_offsets) {
  ...
  // few bits, just do a single parallel count sort
    ...
    size_t num_bkts =
        (num_buckets == 0) ? (size_t{1} << key_bits) : num_buckets;

    // only uses one bucket optimization (last argument) if inplace
    std::tie(offsets, one_bucket) =
        count_sort<assignment_tag>(In, Out, make_slice(get_bits), num_bkts,
    ...
    std::tie(offsets, one_bucket) = count_sort<assignment_tag>(
        In, Out, make_slice(get_bits), num_outer_buckets, parallelism,
        !return_offsets);
    ...
    parallel_for(
        ...
        [&](size_t i) {
          ...
          auto new_parallelism =
              (parallelism * static_cast<float>(end - start)) /
              static_cast<float>(n + 1);
          r = integer_sort_r<typename std::negation<inplace_tag>::type,
                             uninitialized_relocate_tag>(
...
sequence<size_t> integer_sort_(slice<InIterator, InIterator> In,
  ...
  if (bits == 0) {
    ...
    using key_type = std::invoke_result_t<
        Get_Key, typename slice<InIterator, InIterator>::value_type>;
  ...
  return integer_sort_r<inplace_tag, assignment_tag>(In, Out, Tmp, g, bits,
                                                     num_buckets);
}
...
void integer_sort_inplace(slice<Iterator, Iterator> In, Get_Key const& g,
  ...
  integer_sort_<std::true_type, uninitialized_relocate_tag>(In, make_slice(Tmp),
                                                            In, g, bits, 0);
}
