...
void seq_count_(InSeq In, KeySeq Keys, CountIterator counts,
                size_t num_buckets) {
...
std::pair<sequence<size_t>, bool> count_sort_(
  ...
  // size_t par_lower = 1 + static_cast<size_t>(round(num_threads * parallelism
  ...
  if (n < SEQ_THRESHOLD || num_blocks == 1 || num_threads == 1) {
    return std::make_pair(
        seq_count_sort<assignment_tag>(In, Out, Keys, num_buckets), false);
  }
...
auto group_by_small_int(slice<InIterator, InIterator> In,
  ...
  auto results = map(
      total_counts, [](size_t cnt) { return sequence<T>::uninitialized(cnt); });
...
auto count_sort(slice<InIterator, InIterator> In,
  ...
  if (n < max32 && num_buckets < max32)
    ...
    return count_sort_<assignment_tag, uint32_t>(In, Out, Keys, num_buckets,
...
auto count_sort(slice<InIterator, InIterator> In, KeyS const& Keys,
  ...
  auto a = count_sort<uninitialized_copy_tag>(In, make_slice(Out),
                                              make_slice(Keys), num_buckets);
