...
struct diffencoded_entry_encoder {
	...
	struct encoder {
		...
		static inline size_t encoded_size(ET *data, size_t size)
		{
			...
			size_t key_bytes =
				sizeof(K); // first key is uncompressed
			for (size_t i = 0; i < size; i++) {
				K cur_key = Entry::get_key(data[i]);
				K next_diff = cur_key - prev_key;
				auto bytes_used = encodeUnsigned<K>(
					(uint8_t *)stk, 0,
					next_diff); // compressed difference
			...
			if constexpr (is_aug) {
				...
				*((K *)key_bytes) = prev_key; // store first key
				...
				for (size_t i = 1; i < size; i++) {
					...
					K cur_key = Entry::get_key(data[i]);
					K next_diff = cur_key - prev_key;
					offset = encodeUnsigned<K>(
						key_bytes, offset,
						next_diff); // compressed
							    // difference
					prev_key = cur_key;
					av = Entry::combine(
						std::move(av),
						Entry::from_entry(data[i]));
				}
				return av;
			} else {
				K prev_key = Entry::get_key(data[0]);
		...
		{
			throw("errror: reorder not implemented for "
			      "diffencoded_entry_encoder");
		}
		...
		template <class F>
		static inline void inplace_update(uint8_t *bytes, size_t size,
						  F const &f)
		{
			V *vals = (V *)bytes;
			uint8_t *key_bytes = (bytes + size * sizeof(V));
					       ...
					       F const &f)
		{
			...
			K prev_key = *((K *)key_bytes);
			if (!f(Entry::to_entry(prev_key, vals[0])))
				return false;
			key_bytes += sizeof(K);
			for (size_t i = 1; i < size; i++) {
				prev_key += decodeUnsigned<K>(key_bytes);
				if (!f(Entry::to_entry(prev_key, vals[i])))
					return false;
			}
			return true;
		}
...
struct default_entry_encoder {
	...
	struct encoder {
		...
		static inline auto encode(T *data, size_t size, uint8_t *bytes)
		{
			if constexpr (is_aug) {
				...
				for (size_t i = 1; i < size; i++) {
					...
					// data[i]);
					// parlay::assign_uninitialized(ets[i],
					// data[i]);
					parlay::assign_uninitialized(
						ets[i],
						basic_node_helpers::
					...
					auto new_av = Entry::combine(
						std::move(av),
						std::move(next_av));
					// assert(new_av.ref_cnt() == 1);
					av = std::move(new_av);
					// total_size +=
					// std::get<1>(ets[i]).size();
				}
				// std::cout << "Total size in encoded block = "
				// << total_size << std::endl;
				//  assert(av.ref_cnt() == 1);
				return av;
			} else {
		...
		template <class F>
		static inline void append(uint8_t *append_bytes,
					  uint8_t *cur_bytes,
