...
struct balance_utils {
	...
	static regular_node *
	regular_right_join(regular_node *t1, regular_node *t2, regular_node *k)
	{
		...
		if (Node::is_left_heavy(t->rc, t->lc)) {
			if (Node::is_single_rotation(
				    Node::cast_to_regular(t->rc), 0))
				t = rotate_left(t);
			else
	...
	static regular_node *
	regular_left_join(regular_node *t1, regular_node *t2, regular_node *k)
	{
	...
	static regular_node *
	regular_balanced_join(regular_node *l, regular_node *r, regular_node *e)
	{
		e->lc = l;
		e->rc = r;
		Node::update(e);
	...
	// main function
	static regular_node *
	regular_node_join(regular_node *t1, regular_node *t2, regular_node *k)
	{
		if (Node::is_left_heavy(t1, t2))
			return validate(regular_right_join(t1, t2, k));
		if (Node::is_left_heavy(t2, t1))
			return validate(regular_left_join(t1, t2, k));
		return validate(regular_balanced_join(t1, t2, k));
	}
		...
		if (Node::is_left_heavy(t->rc, t->lc)) {
			...
			if (Node::is_single_rotation(
				...
				if (Node::is_compressed(
					    Node::cast_to_regular(t->rc)->lc)) {
					Node::update(t);
					auto ret = Node::make_compressed(
						t, nullptr, nullptr);
					return validate(ret);
				}
				t = double_rotate_left(t);
	...
	// Assumes that e will be used. need to decrement if it is not (using
	// gc)
	static node *balanced_join(node *l, node *r, regular_node *e)
	{
		size_t l_s = Node::size(l), r_s = Node::size(r);
		assert(!Node::is_left_heavy(l, r));
		assert(!Node::is_left_heavy(r, l));

		size_t tot = l_s + r_s + 1;
		// upper_threshold \approx 3.45B
		// size_t upper_threshold = B*(1 + (static_cast<double>(1) /
		// wb_ratio)); 2.45B + 1 size_t upper_threshold = ceil(B *
		// (static_cast<double>(1) / wb_ratio)) + 1; size_t
		// upper_threshold = ceil(B*(1 + (static_cast<double>(1) /
		// wb_ratio)));

		// First check conditions for compressing.
		// If either left or right side is regular, and the overall tree
		// will be at least B, re-make the whole thing (the left  or
		// right side needs to be compressed anyway).
		if ((l_s < B || r_s < B) && (tot >= B)) {
			auto ret = Node::make_compressed(l, r, e);
			if (Node::is_regular(ret)) {
				assert(Node::is_balanced(
					Node::cast_to_regular(ret)));
			}
			return validate(ret);
		}
				  ...
				  regular_node *k)
	{
		...
		size_t upper_threshold =
			ceil(B * (1 + (static_cast<double>(1) / wb_ratio)));
...
#endif
		if (Node::is_left_heavy(t1, t2))
			return validate(right_join(t1, t2, k));
		if (Node::is_left_heavy(t2, t1))
			return validate(left_join(t1, t2, k));
	...
	{
		bool t1_reg = Node::is_regular(t1),
		     t2_reg = Node::is_regular(t2);
#ifdef DEBUG
		if (!Node::check_structure(t1)) {
			assert(false);
		}
		if (!Node::check_structure(t2)) {
			assert(false);
		}
#endif

		if (t1_reg && t2_reg) {
			auto ret = regular_join(Node::cast_to_regular(t1),
						Node::cast_to_regular(t2),
			...
			// Node::reorder(t2);
			if (Node::is_left_heavy(t1, t2)) {
				auto ret = right_join(Node::cast_to_regular(t1),
						      t2, k);
				return validate(ret);
			} else {
				if (Node::is_left_heavy(t2, t1)) {
					auto ret = Node::make_compressed(t1, t2,
									 k);
					return validate(ret);
				} else {
					auto ret = balanced_join(t1, t2, k);
		...
		// Node::reorder(t1);
		// Node::reorder(t2);
		bool balanced = !(Node::is_left_heavy(t1, t2) ||
				  Node::is_left_heavy(t2, t1));
