...
struct map_ops : Seq {
	...
	{
		if (a.empty())
			return split_info(NULL, std::nullopt, NULL);
	...
	static node *uniont(ptr b1, ptr b2, BinaryOp const &op)
	{
		...
		if (sp1.mid)
			combine_values(m2, *sp1.mid, true, op);
			       ...
			       BinaryOp const &op)
	{
		...
		auto [l, r] = utils::fork<node *>(
			Seq::do_parallel(n1, n2),
			[&]() {
				return intersect<Seq1, Seq2>(sp1.l,
							     std::move(l2), op);
			},
			[&]() {
				return intersect<Seq1, Seq2>(sp1.r,
							     std::move(r2), op);
			});
	...
	static node *range_root_2(node *b, K const &key_left,
				  K const &key_right)
	{
		if (!b)
			return NULL;
	...
	static node *range_2(node *b, K const &low, K const &high)
	{
		...
		if (Seq::is_compressed(r)) {
			...
			return Seq::join(right_2(lc, low), e, left_2(rc, high),
					 nullptr);
		}
	}
		...
		auto merge = [&](et_type const &et) {
			if (!placed) {
				if (Entry::comp(Entry::get_key(et), key)) {
					...
					parlay::assign_uninitialized(
						merged[out_off], et);
					combine_values(merged[out_off], e, true,
						       f);
					out_off++;
					placed = true;
				}
				 ...
				 J const &join)
	{
		...
		regular_node *o =
			Seq::cast_to_regular(make_node_tmpl<copy>(br));
		if (Entry::comp(get_key(br), Entry::get_key(e))) {
			node *r =
				insert_tmpl<Func, J, copy>(br->rc, e, f, join);
	...
	static node *my_remove_2(ptr b, K const &k, J const &join)
	{
		...
		if (Entry::comp(ek, k)) {
			regular_node *o =
				(root != nullptr) ? root : Seq::single(e);
	...
	template <class Func, class J, bool copy = false>
	static node *update_tmpl(node *b, K const &k, Func const &f,
				 J const &join)
	{
		if (!b)
			return b;
		...
		if (Entry::comp(get_key(br), k)) {
			regular_node *o =
				Seq::cast_to_regular(make_node_tmpl<copy>(br));
			node *r =
				update_tmpl<Func, J, copy>(br->rc, k, f, join);
			return join(inc_tmpl<copy>(br->lc), r, o);
		} else if (Entry::comp(k, get_key(b))) {
			regular_node *o =
				Seq::cast_to_regular(make_node_tmpl<copy>(br));
			node *l =
				update_tmpl<Func, J, copy>(br->lc, k, f, join);
			return join(l, inc_tmpl<copy>(br->rc), o);
		} else {
	...
	static node *multi_delete_sorted(ptr b, K *A, size_t n)
	{
		...
		auto P = utils::fork<node *>(
			...
			[&]() {
				return multi_delete_sorted(std::move(lc), A,
							   mid);
			},
			[&]() {
				return multi_delete_sorted(std::move(rc),
							   A + mid + dup,
							   n - mid - dup);
			});
	...
	static node *multi_diff_sorted(ptr b, K *A, size_t n)
	{
		...
		auto P = utils::fork<node *>(
			...
			[&]() {
				return multi_diff_sorted(std::move(rc),
							 A + mid + dup,
							 n - mid - dup);
			});
	...
	// map: VE -> V
	template <class VE, class CombineOp>
	static node *multi_delete_sorted_map(ptr b, std::pair<K, VE> *A,
					     size_t n,
					     CombineOp const &combine_op)
	{
		if (b.empty())
		...
		auto P = utils::fork<node *>(
			true, // Seq::do_parallel(b.size(), n),
			[&]() {
				return multi_delete_sorted_map(std::move(lc), A,
							       mid, combine_op);
			},
			[&]() {
				return multi_delete_sorted_map(
					std::move(rc), A + mid + dup,
					n - mid - dup, combine_op);
			});

		if (dup)
			combine_values_v(root, A[mid].second, combine_op);
		return Seq::node_join(P.first, P.second, root);
	}
					 ...
					 BinaryOp const &op)
	{
		...
		if (b.is_compressed()) {
			if (tot <= 2 * B) { // can put in one leaf directly
				...
				return multiinsert_compress(std::move(b), A, n,
							    op);
			} else {
				;
			}
		...
		auto P = utils::fork<node *>(
			...
			[&]() {
				return multi_insert_sorted(std::move(lc), A,
							   mid, op);
			},
			[&]() {
				return multi_insert_sorted(std::move(rc),
							   A + mid + dup,
							   n - mid - dup, op);
			});
						...
						BinaryOp const &op)
	{
		...
		// auto [lc, e, rc, root] = Seq::expose(std::move(b));
		auto rt = Seq::is_regular(b)
				  ? Seq::cast_to_regular(b)
		...
		auto P = utils::fork<node *>(
			...
			// Seq::do_parallel(b.size(), n),
			[&]() {
				return multi_insert_sorted_simple(rt->lc, A,
								  mid, op);
			},
			[&]() {
				return multi_insert_sorted_simple(
					rt->rc, A + mid + dup, n - mid - dup,
					op);
			});
	...
	// map: VE -> V
	template <class VE, class CombineOp, class MapOp>
	static node *multi_insert_sorted_map(ptr b, std::pair<K, VE> *A,
					     size_t n,
					     CombineOp const &combine_op,
					     MapOp const &map_op)
	{
		...
		auto P = utils::fork<node *>(
			true, // Seq::do_parallel(b.size(), n),
			[&]() {
				return multi_insert_sorted_map(std::move(lc), A,
							       mid, combine_op,
							       map_op);
			},
			[&]() {
						  ...
						  size_t n, BinaryOp const &op)
	{
		...
		auto update_f = [&](et_type et) {
			...
			if (ct < n) {
				K key_a = A[ct].first;
				while (Entry::comp(key_a, key_et) &&
				       ct < n - 1) {
					ct++;
					key_a = A[ct].first;
				}
				if (!Entry::comp(key_a, key_et) &&
				    !Entry::comp(key_et, key_a)) {
					ret_v = op(Entry::get_val(et),
						   A[ct].second);
					ct++;
				}
			}
						...
						size_t n, BinaryOp const &op)
	{
		...
		auto P = utils::fork<bool>(
			true,
			[&]() {
				return multi_update_sorted_inplace(
					std::move(rb->lc), A, mid, op);
			},
			[&]() {
				return multi_update_sorted_inplace(
					std::move(rb->rc), A + mid + dup,
					n - mid - dup, op);
			});

		if (dup)
			combine_values_v(rb, A[mid].second, op);
				      ...
				      size_t offset)
	{
		...
		utils::fork<bool>(
			true,
			[&]() {
				return multi_find_sorted(rb->lc, A, mid, ret,
							 offset);
			},
			[&]() {
					  ...
					  BinaryOp const &op)
	{
		...
		// NOTE: the new returned tree replace the n_b1, decrease the
		// counter
		Seq::decrement_recursive(n_b1);
