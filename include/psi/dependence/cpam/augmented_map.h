...
public:
	...
	static size_t range_count_filter2(M &m, Box const &query_box,
					  Logger &logger)
	{
		return Tree::template range_count_filter2<BaseTree, Box,
							  Logger>(
	...
	{
		Tree::template knn_filter<BaseTree>(m.get_root(), q, f, f2, k,
						    out, logger);
		return;
	}

	template <class BaseTree, typename Box, typename Logger, typename Out>
	static void range_report_filter2(M &m, Box const &query_box,
	...
	static M multi_insert_sorted_map(M m, Seq &SS, CombineOp combine_op,
					 MapOp map_op)
	{
		return to_aug(Map::multi_insert_sorted_map(std::move(m), SS,
							   combine_op, map_op));
	}
	template <class Seq, class CombineOp>
	static M multi_delete_sorted_map(M m, Seq &SS, CombineOp combine_op)
	{
		return to_aug(Map::multi_delete_sorted_map(std::move(m), SS,
							   combine_op));
	}
	...
	{
		return to_aug(
			Map::map_intersect(std::move(a), std::move(b), op));
	}
	...
	{
		return to_aug(Map::range(a, kl, kr));
	}
