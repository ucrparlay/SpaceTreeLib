...
public:
	using map_type = map_<_Entry, Join_Tree>;
	using Entry = typename map_type::Entry;
	using Tree = augmented_ops<typename map_type::Tree>;
	...
	static size_t range_count_filter2(M &m, box_type const &query_box,
					  Logger &logger)
	{
		return Tree::template range_count_filter2<base_tree, box_type,
							  Logger>(
	...
	static void knn(M &m, E const &q, bounded_queue &bq, Logger &logger)
	{
		// parlay::internal::timer t;
		// t.start();
		// auto root = m.get_root();
		// logger.generate_box_num += t.total_time();
		// Tree::template knn<base_tree>(m.get_root(), q, bq, logger);
		Tree::template knn<base_tree>(m.root, q, bq, logger);
		// gc_type::decrement(root);
	}

	template <class base_tree, typename F, typename F2, typename Out,
		  typename Logger>
	static void knn_filter(M m, E const &q, F const &f, const F2 &f2,
			       size_t k, Out &out, Logger &logger)
	{
		Tree::template knn_filter<base_tree>(m.get_root(), q, f, f2, k,
						     out, logger);
		return;
	}
	...
	template <class base_tree, typename box_type, typename Logger,
		  typename Out>
	static void range_report_filter2(M &m, box_type const &query_box,
					 size_t &cnt, Out &&out, Logger &logger)
	{
		Tree::template range_report_filter2<base_tree>(
			// m.get_root(), query_box, cnt, std::forward<Out>(out),
			// logger);
	...
	A aug_val() const
	{
		return Tree::aug_val(map_type::root);
	}

	A aug_left(K const &key)
	{
		typename Tree::aug_sum_t a;
		Tree::aug_sum_left(map_type::root, key, a); // NOT using ptr.
		return a.result;
	}
	...
	{
		typename Tree::aug_sum_t a;
		Tree::aug_sum_right(map_type::root, key, a);
		return a.result;
	}
	...
	{
		typename Tree::aug_sum_t a;
		Tree::aug_sum_range(map_type::root, key_left, key_right, a);
		return a.result;
	}

	// just side effecting
	template <class restricted_sum>
	void range_sum(K const &key_left, K const &key_right,
		       restricted_sum &rs)
	{
		Tree::aug_sum_range(map_type::root, key_left, key_right, rs);
	}

	template <class Func>
	maybe_E aug_select(Func f)
	{
		return Tree::aug_select(map_type::root, f);
	}
	...
	// install map_type's constructors
	using map_type::map_type;

	template <class Func>
	static M insert(M m, E const &p, Func const &f)
	{
		return to_aug(map_type::insert(std::move(m), p, f));
	}
	static M insert(M m, E const &p)
	{ // cout << "ins a " << endl;
		return to_aug(map_type::insert(std::move(m), p));
	}
	static M remove(M m, K const &k)
	{
		return to_aug(map_type::remove(std::move(m), k));
	}
	template <class F>
	static M filter(M m, F const &f)
	{
		return to_aug(map_type::filter(std::move(m), f));
	}
	static M multi_insert(M m, parlay::sequence<E> const &SS)
	{
		return to_aug(map_type::multi_insert(std::move(m), SS));
	}
	template <typename slice_type>
	static M multi_insert(M m, slice_type const &SS)
	{
		return to_aug(map_type::multi_insert(std::move(m), SS));
	}
	...
	{
		return Tree::multi_update_sorted_inplace(m.root, SS.begin(),
							 SS.size(), f);
	}
	...
	{
		return map_type::multi_find_sorted(m, SS);
	}
	...
	{
		return to_aug(map_type::multi_insert_combine(std::move(m), S, f,
							     seq_inplace));
	}
	template <class Val, class Reduce>
	static M multi_insert_reduce(M m,
				     parlay::sequence<pair<K, Val>> const &S,
				     Reduce g)
	{ // ?? should it be &
		return to_aug(
			map_type::multi_insert_reduce(std::move(m), S, g));
	}

	template <class M1, class M2, class F>
	static M map_intersect(M1 a, M2 b, F const &op)
	{
		return to_aug(map_type::map_intersect(std::move(a),
						      std::move(b), op));
	}
	static M map_intersect(M a, M b)
	{
		return to_aug(
			map_type::map_intersect(std::move(a), std::move(b)));
	}
	template <class F>
	static M map_union(M a, M b, F const &op)
	{
		return to_aug(
			map_type::map_union(std::move(a), std::move(b), op));
	}
	static M map_union(M a, M b)
	{
		return to_aug(map_type::map_union(std::move(a), std::move(b)));
	}
	static M map_difference(M a, M b)
	{
		return to_aug(
			map_type::map_difference(std::move(a), std::move(b)));
	}

	static M range(M &a, K kl, K kr)
	{
		return to_aug(map_type::range(a, kl, kr));
	}
	template <class Ma, class F>
	static M map(Ma &a, F const f)
	{
		return to_aug(map_type::map(a, f));
	}
	static M subseq(M &a, size_t left, size_t right)
	{
		return to_aug(map_type::subseq(a, left, right));
	}

	static size_t size(node *r)
	{
		return map_type::size(r);
	}
	...
	bool operator==(M const &m)
	{
		return map_type::operator==(m);
	}
	...
	{
		return to_aug(map_type::map_set(a, f));
	}
	...
	{
		map_type::foreach_cond_par(m, f, c);
	}
	// apply function f on all entries sequentially
	template <class F>
	static void foreach_seq(M const &m, F const &f)
	{
		map_type::foreach_seq(m, f);
	}

public:
	using map_type::check_balance;
	using map_type::check_structure;
	using map_type::clear;
	using map_type::contains;
	using map_type::find;
	using map_type::finish;
	using map_type::from_sorted;
	using map_type::get_root;
	using map_type::init;
	using map_type::insert;
	using map_type::is_empty;
	using map_type::iterate_seq;
	using map_type::node_stats;
	using map_type::rank;
	using map_type::ref_cnt;
	using map_type::reserve;
	using map_type::root;
	using map_type::root_is_compressed;
	using map_type::select;
	using map_type::size;
	using map_type::size_in_bytes;
	using map_type::subseq;
	using map_type::update;
};
...
struct aug_map_full_entry : entry {
	...
	static inline auto get_key(entry_t const &e)
	{
		return entry::get_key(e);
	}
	static inline auto get_val(entry_t const &e)
	{
		return entry::get_val(e);
	}
...
using aug_map = aug_map_<
	aug_map_full_entry<_Entry>,
	typename Balance::template balance<aug_node<
		typename Balance::data, aug_map_full_entry<_Entry>,
		typename Encoder::template encoder<aug_map_full_entry<_Entry>,
						   /* is_aug = */ true>,
		BlockSize>>>;
...
struct aug_set_full_entry : entry {
	...
	using aug_t = typename entry::aug_t;
	using entry_t = key_t;
	static inline key_t get_key(entry_t const &e)
	{
		return e;
	}
	static inline val_t get_val(entry_t const &e)
	{
		return 0;
	}
	static inline void set_val(entry_t &e, val_t const &v)
...
template <class _Entry, size_t BlockSize = 256,
	  class Encoder = default_entry_encoder,
	  class Balance = weight_balanced_tree>
using aug_set = aug_map_<
	aug_set_full_entry<_Entry>,
	typename Balance::template balance<aug_node<
		typename Balance::data, aug_set_full_entry<_Entry>,
		typename Encoder::template encoder<aug_set_full_entry<_Entry>,
						   /* is_aug = */ true>,
		BlockSize>>>;
