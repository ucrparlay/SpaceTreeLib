...
struct weight_balanced_tree {
	...
	struct balance : public Node {
		...
		static regular_node *regular_node_join(regular_node *t1,
						       regular_node *t2,
						       regular_node *k)
		{
		...
		static inline bool is_balanced(regular_node *t)
		{
			...
			// std::cout << "lc_heavy = " << lc_heavy << " rc_heavy
			// = " << rc_heavy << std::endl;
			bool ok = !t || !(lc_heavy || rc_heavy);
			// check tight balance constraint on nodes with
			// compressed children
			//      if (t) {
			//        if (Node::is_compressed(t->lc) ||
			//        Node::is_compressed(t->rc)) {
			//          bool l_ok = is_single_rotation(t, 0);
			//          bool r_ok = is_single_rotation(t, 1);
			//          if (!r_ok || !l_ok) {
			//            std::cout << "t weight =" << weight(t) <<
			//            std::endl; std::cout << "l weight =" <<
			//            weight(t->lc) << std::endl; std::cout <<
			//            "r weight =" << weight(t->rc) <<
			//            std::endl;
			//          }
			//          assert(l_ok);
			//          assert(r_ok);
		...
		static bool
		check_structure(node *b,
				size_t orig_tree_size =
	...
	private:
		...
		// if s_t1 = 9, s_t2 = 1, then below inequality holds, then leaf
		// heavy
		...
		static inline bool could_single_rotate(node *lc, node *rc)
		{
			...
			// std::cout << "beta = " << beta << " ls = " << l_s <<
			// " rs = " << r_s << " tot = " << tot << std::endl;
