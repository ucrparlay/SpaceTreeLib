...
public:
	...
	static compressed_node *make_single_compressed_node(ET *e, size_t s)
	{
		...
		compressed_node *c_node =
			(compressed_node *)utils::new_array_no_init<uint8_t>(
				node_size);
		// compressed_node* c_node =
		// (compressed_node*)complex_allocator::alloc();
		...
		uint8_t *encoded_data =
			(((uint8_t *)c_node) + sizeof(compressed_node));
	...
	static void print_node_info(node *a, std::string node_name)
	{
		...
		//      " << get_entry(a).first << " Size = " << size(a) << "
		//      left child = " << size(r->lc) << " right_child = " <<
		//      size(r->rc) <<  std::endl;
		//    } else {
		//      auto c = cast_to_compressed(a);
		//      std::cout << "Node " << node_name << " is compressed.
		//      Size = " << c->s << std::endl;
		//    }
	}
