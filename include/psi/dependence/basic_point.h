...
struct BasicPoint {
	...
	explicit BasicPoint(T const val)
	{
		this->pnt.fill(val);
	}
	...
	bool SameDimension(BasicPoint const &b) const
	{
		return *this == b;
	}
...
struct AugPoint : BasicPoint<T, d> {
	...
	AugPoint(typename BasicPoint<T, d>::Coords const &_pnt,
		 AugType const &aug)
	...
	bool operator<(AugPoint const &x) const
	{
		return BasicPoint<T, d>::operator<(x)
			       ? true
			       : (BasicPoint<T, d>::operator==(x) ? aug < x.aug
								  : false);
	}

	friend std::ostream &operator<<(std::ostream &o, AugPoint const &a)
	{
		o << "[" << static_cast<BasicPoint<T, d> const &>(a) << " -> "
		  << a.aug << "] " << std::flush;
		return o;
	}
	...
	{
		return static_cast<BP const &>(*this).MinCoords(
			static_cast<BP const &>(b));
	}

	BP MaxCoords(AugPoint const &b) const
	{
		return static_cast<BP const &>(*this).MaxCoords(
			static_cast<BP const &>(b));
	}
