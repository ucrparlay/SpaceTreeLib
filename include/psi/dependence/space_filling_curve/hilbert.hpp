...
#if defined(DLOGB_BIT_TRANSPOSE)
{
	...
	while ((utB = inB / 2)) {
		...
		unsigned d;
		if (inB & 1) {
			bitmask_t const inFieldStarts = inFieldEnds
							<< (inB - 1);
 ...
 * (bits_per_byte)
 */
inline void hilbert_i2c(unsigned nDims, unsigned nBits, bitmask_t index,
			bitmask_t coord[])
{
	if (nDims > 1) {
		bitmask_t coords;
		...
		if (nBits > 1) {
			...
			halfmask_t const nd1Ones =
				ndOnes >> 1; /* for adjust_rotation */
			unsigned b = nDimsBits;
			unsigned rotation = 0;
			halfmask_t flipBit = 0;
			bitmask_t const nthbits =
				ones(bitmask_t, nDimsBits) / ndOnes;
			index ^= (index ^ nthbits) >> 1;
			coords = 0;
			do {
				halfmask_t bits =
					(index >> (b -= nDims)) & ndOnes;
				coords <<= nDims;
				coords |= rotateLeft(bits, rotation, nDims) ^
					  flipBit;
			     ...
			     bitmask_t const coord[])
{
	if (nDims > 1) {
		...
		if (nBits > 1) {
			...
			do {
				halfmask_t bits =
					(coords >> (b -= nDims)) & ndOnes;
				bits = rotateRight(flipBit ^ bits, rotation,
						   nDims);
				index <<= nDims;
				index |= bits;
				flipBit = (halfmask_t)1 << rotation;
			      ...
			      bitmask_t *index)
{
	...
	if (y % 2 == 1) {
		*rotation = (IEEErepBits - y + 1 + leastZeroBit) % nDims;
		...
		if (y < IEEErepBits) {
			unsigned shift_amt =
				(IEEErepBits - y + leastZeroBit) % nDims;
				     ...
				     bitmask_t index, BitReader getBits)
{
	...
	while (y--) {
		...
		if (diff) {
			unsigned d;
			bitmask_t smear =
				rotateRight(diff, rotation, nDims) >> 1;
			bitmask_t digit =
...
inline unsigned hilbert_box_vtx(unsigned nDims, unsigned nBytes, unsigned nBits,
...
inline unsigned hilbert_ieee_box_vtx(unsigned nDims, int findMin, double *c1,
				     double *c2)
{
	unsigned rotation, max;
	bitmask_t bits, index;
 ...
 * * (bits_per_byte)
 */
inline unsigned hilbert_box_pt_work(unsigned nDims, unsigned nBytes,
				    unsigned nBits, int findMin, unsigned max,
				    unsigned y, char *c1, char *c2,
				    unsigned rotation, bitmask_t bits,
				    bitmask_t index, BitReader getBits,
...
inline unsigned hilbert_box_pt(unsigned nDims, unsigned nBytes, unsigned nBits,
			       int findMin, void *c1, void *c2)
{
	bitmask_t const one = 1;
	bitmask_t bits = one << (nDims - 1);
...
inline unsigned hilbert_ieee_box_pt(unsigned nDims, int findMin, double *c1,
				    double *c2)
{
	unsigned rotation, max;
	bitmask_t bits, index;
	...
	if (c1Signs != c2Signs) {
		...
		index = 1;
		hilbert_box_pt_work(nDims, 8, 64, findMin, IEEErepBits - 1,
	...
	return hilbert_box_pt_work(nDims, 8, 64, findMin, max, max + 53,
				   (char *)c1, (char *)c2, rotation, bits,
				   index, getIEEEBits, propogateIEEEBits);
}
 ...
 *      nBits <= (sizeof bitmask_t) * (bits_per_byte)
 */
inline int hilbert_nextinbox(unsigned nDims, unsigned nBytes, unsigned nBits,
			     int findPrev, void *c1V, void *c2V,
			     void const *ptV)
{
	bitmask_t const one = 1;
	unsigned y = nBits;
	...
	while (y-- > 0) {
		   ...
		   diffed dimensions don't matter. */
		{
			...
			bitmask_t cornerdiff =
				(diff ^ reflection) ^ /* separate box
							 crnrs */
				((getIntBits(nDims, nBytes, c2, y) & ~fold2) |
				 valu2);
			...
			/* bits less significant than the msb of
			   ...
			   too */
			bitmask_t rotSep =
				rotateRight(separator, rotation, nDims);
			   ...
			   there, skip it */
			{
				...
				p_separator = rotateLeft(p_separator, rotation,
							 nDims);
				p_firstSeparator = rotateLeft(p_firstSeparator,
							      rotation, nDims);
				p_cornerdiff = cornerdiff & ~(p_separator ^
							      p_firstSeparator);
				p_y = y;
			   ...
			   it on the curve; almost done */
			{
				...
				hilbert_box_pt(nDims, nBytes, nBits, !findPrev,
					       c1V, c2V);
				return 1;
			}
		}
	...
	for (;;) {
		...
		for (r = 0; r < nPrints; ++r) {
			...
			for (r1 = 0; r1 < r; ++r1) {
				...
				ans = hilbert_cmp(nDims, sizeof(coord[0]),
						  nBits, coord, coord1);
				if (ans != 1) {
					...
					printf("cmp r = "
					       ...
					       "0x%0*x, ans = "
					       "%2d\n",
					       width, (unsigned)r, width,
					       (unsigned)r1, ans);
				}
			}
			hilbert_i2c(nDims, nBits, r1, coord1);
			if (hilbert_cmp(nDims, sizeof(coord[0]), nBits, coord,
					coord1) != 0)
...
{
	return hilbert_cmp(g_nDims, sizeof(unsigned), 8 * sizeof(unsigned), c1p,
			   c2p);
}
...
int main()
{
	...
	hilbert_box_vtx(nDims, sizeof(unsigned), 8 * sizeof(unsigned), 1,
			cornerlo, work);
...
int main()
{
	...
	hilbert_box_pt(nDims, sizeof(unsigned), 8 * sizeof(unsigned), 1,
		       pointlo, work);
...
int main()
{
	...
	stat = hilbert_nextinbox(nDims, nBytes, 8 * nBytes, findPrev, c1, c2,
				 pt);
