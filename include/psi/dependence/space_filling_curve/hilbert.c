...
#define adjust_rotation(rotation, nDims, bits)           \
  do {                                                   \
    ...
    if (++rotation >= nDims) rotation -= nDims;          \
  } while (0)
    ...
    void hilbert_i2c(unsigned nDims, unsigned nBits, bitmask_t index,
      ...
      if (nDims > 1) {
        ...
        if (nBits > 1) {
          ...
          for (b = nDims; b < nDimsBits; b *= 2) coords ^= coords >> b;
    ...
    bitmask_t hilbert_c2i(unsigned nDims, unsigned nBits,
      ...
      if (nDims > 1) {
        ...
        if (nBits > 1) {
          ...
          index = coords;
        for (d = 1; d < nDimsBits; d *= 2) index ^= index >> d;
        return index;
      } else
        return coord[0];
    }
...
#include <string.h>
    static void propogateIntBits(unsigned d, unsigned nBytes, char* c,
                                 unsigned y, int fold) {
    ...
    /* An IEEE double is treated as a 2100 bit number.  In particular, 0 is
       treated as a 1 followed by 2099 zeroes, and negative 0 as a 0 followed by
       2099 ones. Only 53 bits differ between a number and a zero of the same
       sign, with the position of the 53 determined by the exponent, and the
       values of the 53 by the significand (with implicit leading 1 bit).
       Although IEEE 754 uses the maximum exponent for NaN's and infinities,
       this implementation ignores that decision, so that infinities and NaN's
       are treated as very large numbers. Note that we do not explicitly
       construct a 2100 bit bitmask in the IEEE routines below. */
    ...
    static void propogateIEEEBits(unsigned d, unsigned nBytes, char* cP,
                                  unsigned y, int fold) {
    ...
    static unsigned getIEEEexptMax(unsigned nDims, double const* c) {
      ...
      for (d = 0; d < nDims; ++d) {
        ieee754_double x;
        x.d = c[d];
        if (max < x.ieee.exponent) max = x.ieee.exponent;
      }
      if (max) --max;
      return max;
    }
    ...
    static void getIEEEinitValues(double const* c1, unsigned y, unsigned nDims,
      ...
      /* compute the odd/evenness of the number of sign bits */
      {
        ...
        for (d = 1; d < nDims; d *= 2) signPar ^= signPar >> d;
      ...
      for (leastZeroBit = 0; leastZeroBit < nDims; ++leastZeroBit)
        if (rdbit(signBits, leastZeroBit) == 0) break;
      strayBit = 0;
    ...
    static int hilbert_cmp_work(
        unsigned nDims, unsigned nBytes, unsigned nBits, unsigned max,
        unsigned y, char const* c1, char const* c2, unsigned rotation,
     ...
     * that contains the selected corner and no other corners Assumptions: nBits
     * <= (sizeof bitmask_t) * (bits_per_byte)
     */

    static unsigned hilbert_box_vtx_work(
        unsigned nDims, unsigned nBytes, unsigned nBits, int findMin,
        unsigned max, unsigned y, char* c1, char* c2, unsigned rotation,
        bitmask_t bits, bitmask_t index, BitReader getBits) {
      bitmask_t const one = 1;
      ...
      while (y--) {
        ...
        if (diff) {
          ...
          index &= 1;
          if ((index ^ y ^ findMin) & 1) digit ^= smear + 1;
     ...
     * hilbert_nextinbox
     *
     * Determine the first point of a box after or before a given point to lie
     on
     * a Hilbert curve
     * Inputs:
     *  nDims:      Number of coordinates.
    ...
    int hilbert_nextinbox(unsigned nDims, unsigned nBytes, unsigned nBits,
      ...
      while (y-- > 0) {
           ...
           dimensions don't matter. */
        {
          ...
          index &= 1;
          digit &= rotSep;
          if ((index ^ y ^ findPrev) & 1) digit ^= rotSep;
             ...
             ahead of it on the curve; almost done */
          {
            ...
            for (d = 0; d < nDims; ++d) {
              ...
              if (rdbit(separator, d)) hibits ^= bthbit;
              if (rdbit(firstSeparator, d)) hipart ^= bthbit;
    ...
    void hilbert_incr(unsigned nDims, unsigned nBits, bitmask_t coord[]) {
      ...
      for (b = nBits; b--;) {
        ...
        reflection = 0;
        for (d = 0; d < nDims; ++d) reflection |= rdbit(coord[d], b) << d;
        bits ^= reflection;
        bits = rotateRight(bits, rotation, nDims);
        index ^= bits;
        for (d = 1; d < nDims; d *= 2) index ^= index >> d;
        if (index++ != ndOnes) {
          rb = b;
          rd = index & -index;
    ...
    /* LICENSE
     *
     * This software is copyrighted by Rice University.  It may be freely
     * copied, modified, and redistributed, provided that the copyright notice
    ...
    /* Revision history:
       ...
       Dec 1998: Fixed bug in hilbert_c2i that allowed a shift by number of bits
       in bitmask to vaporize index, in last bit of the function.  Implemented
       hilbert_incr.

       August 1999: Added argument to hilbert_nextinbox so that you can,
       optionally, find the previous point along the curve to intersect the box,
       rather than the next point.

       Nov 1999: Defined fast bit-transpose function (fast, at least, if the
       number of bits is large), and reimplemented i2c and c2i in terms of it.
       Collapsed loops in hilbert_cmp, with the intention of reusing the cmp
       code to compare more general bitstreams.

       Feb 2000: Implemented almost all the floating point versions of cmp, etc,
       so that coordinates expressed in terms of double-precision IEEE floating
       point can be ordered.  Still have to do next-in-box, though.
          ...
          for (r1 = 0; r1 < r; ++r1) {
            ...
            if (ans != 1) {
              ...
              printf("cmp r = 0x%0*x; r1 = 0x%0*x, ans = %2d\n", width,
                     (unsigned)r, width, (unsigned)r1, ans);
            }
          }
          hilbert_i2c(nDims, nBits, r1, coord1);
...
#define maxDim (8 * sizeof(bitmask_t))
    int main() {
      ...
      for (k = 0; k < (1 << nDims); ++k) {
        printf("Orth %2d\n", k);
        for (i = 0; i < nDims; ++i) coord[i] = ((k >> i) & 1) ? -1. : 1.;
...
#define maxDim (8 * sizeof(bitmask_t))
    ...
    int main() {
      ...
      for (k = 0; k < nDims; ++k) scanf("%d", &corner0[k]);

      printf("Enter other corner (%d coordinates): ", nDims);
      for (k = 0; k < nDims; ++k) scanf("%d", &corner1[k]);
      ...
      for (k = 0; k < nDims; ++k) printf("%4u", cornerlo[k]);
      printf("\n");
      ...
      for (k = 0; k < nDims; ++k) printf("%4u", cornerhi[k]);
      printf("\n");
      ...
      for (k = 0; k < (1 << nDims); k += (1 << nDims) - 1) {
        unsigned j;
        unsigned* eltk = &array[k][0];
        for (j = 0; j < nDims; ++j) printf("%4u", eltk[j]);
        printf("\n");
      }
      free((char*)array);
      return 0;
...
#define maxDim (8 * sizeof(bitmask_t))
    ...
    int main() {
      ...
      for (k = 0; k < nDims; ++k) scanf("%d", &point0[k]);
      ...
      for (k = 0; k < nDims; ++k) printf("%4u", pointlo[k]);
      printf("\n");
      ...
      for (k = 0; k < nDims; ++k) printf("%4u", pointhi[k]);
      printf("\n");

      array =
          (array_t*)malloc(maxDim * sizeof(unsigned) * (outvolume - involume));
      if (array == 0) {
...
#define maxDim (8 * sizeof(bitmask_t))
    ...
    int main() {
      ...
      printf("Enter one point (%d coordinates): ", nDims);
      for (k = 0; k < nDims; ++k) scanf("%lf", &point0[k]);

      printf("Enter other point (%d coordinates, strictly greater): ", nDims);
      for (k = 0; k < nDims; ++k) scanf("%lf", &point1[k]);
      ...
      printf("Predicted hi point: ");
      for (k = 0; k < nDims; ++k) printf("%10lg", pointhi[k]);
      printf("\n");
      ...
      printf("Predicted lo point: ");
      for (k = 0; k < nDims; ++k) printf("%10lg", pointlo[k]);
      printf("\n");
...
#include <stdio.h>
    ...
    int main() {
      ...
      for (i = 0; i < nDims; ++i) scanf("%u", &c1[i]);
      printf("Enter 2nd box corner: ");
      for (i = 0; i < nDims; ++i) scanf("%u", &c2[i]);
      printf("Enter point: ");
      for (i = 0; i < nDims; ++i) scanf("%u", &pt[i]);
      printf("Find prev?: ");
      scanf("%d", &findPrev);
      ...
      if (stat)
        for (i = 0; i < nDims; ++i) printf("%u ", c1[i]);
      else
