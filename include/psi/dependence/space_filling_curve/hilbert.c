...
#define DLOGB_BIT_TRANSPOSE
		 ...
		 * hilbert_i2c
		 *
		 * Convert an index into a Hilbert curve to a set of
		 * coordinates. Inputs: nDims:      Number of coordinate axes.
		 *  nBits:      Number of bits per axis.
		 *  index:      The index, contains nDims*nBits bits
		 *              (so nDims*nBits must be <= 8*sizeof(bitmask_t)).
		 * Outputs:
		 *  coord:      The list of nDims coordinates, each with nBits
			...
			if (nDims > 1) {
				...
				if (nBits > 1) {
					unsigned const nDimsBits =
						nDims * nBits;
					halfmask_t const ndOnes =
						ones(halfmask_t, nDims);
					halfmask_t const nd1Ones =
						ndOnes >>
						1; /* for adjust_rotation */
					unsigned b = nDimsBits;
					unsigned rotation = 0;
					halfmask_t flipBit = 0;
					bitmask_t const nthbits =
						ones(bitmask_t, nDimsBits) /
						ndOnes;
					...
					do {
						halfmask_t bits =
							...
							ndOnes;
						coords <<= nDims;
						coords |= rotateLeft(bits,
								     rotation,
								     nDims) ^
							  flipBit;
						flipBit = (halfmask_t)1
							  << rotation;
						adjust_rotation(rotation, nDims,
								bits);
					} while (b);
					for (b = nDims; b < nDimsBits; b *= 2)
						coords ^= coords >> b;
					coords = bitTranspose(nBits, nDims,
							      coords);
				} else
		 ...
		 * hilbert_c2i
		 *
		 * Convert coordinates of a point on a Hilbert curve to its
		 * index. Inputs: nDims:      Number of coordinates. nBits:
		 * Number of bits/coordinate. coord:      Array of n nBits-bit
		 * coordinates. Outputs: index:      Output index value.
			...
			if (nDims > 1) {
				...
				if (nBits > 1) {
					...
					bitmask_t const nthbits =
						ones(bitmask_t, nDimsBits) /
						ndOnes;
					coords = bitTranspose(nDims, nBits,
							      coords);
					coords ^= coords >> nDims;
					index = 0;
					do {
		...
		typedef void (*BitWriter)(unsigned d, unsigned nBytes, char *c,
					  unsigned y, int fold);
...
#define setBytes(dst, pos, nBytes, val) memset(&dst[0], val, pos)
		...
		static bitmask_t getIntBits(unsigned nDims, unsigned nBytes,
					    char const *c, unsigned y)
		{
...
#include <string.h>
		static void propogateIntBits(unsigned d, unsigned nBytes,
		...
		/* An IEEE double is treated as a 2100 bit number.  In
		   particular, 0 is treated as a 1 followed by 2099 zeroes, and
		   negative 0 as a 0 followed by 2099 ones. Only 53 bits differ
		   between a number and a zero of the same sign, with the
		   position of the 53 determined by the exponent, and the values
		   of the 53 by the significand (with implicit leading 1 bit).
		   Although IEEE 754 uses the maximum exponent for NaN's and
		   infinities, this implementation ignores that decision, so
		   that infinities and NaN's are treated as very large numbers.
		   Note that we do not explicitly construct a 2100 bit bitmask
		   in the IEEE routines below. */
		...
		static bitmask_t getIEEESignBits(unsigned nDims,
						 double const *c)
		{
			unsigned d;
			ieee754_double x;
		...
		/* retrieve bits y of elements of double array c, where an
		   expanded IEEE double has 2100 bits. */
		{
			unsigned d;
		...
		static void propogateIEEEBits(unsigned d, unsigned nBytes,
		...
		static void getIEEEinitValues(
			double const *c1, unsigned y, unsigned nDims,
			unsigned *rotation, bitmask_t *bits, bitmask_t *index)
		{
			...
			/* find the position of the least-order 0 bit in among
			   signBits and adjust it if necessary */
			for (leastZeroBit = 0; leastZeroBit < nDims;
			     ++leastZeroBit)
			...
			if (y % 2 == 1) {
				*rotation =
				...
				{
					*bits = signBits ^
						(ones(bitmask_t, nDims) & ~1);
					*index = signParity ^ (nDims & 1);
				}
			} else /* y % 2 == 0 */
				if (y < IEEErepBits) {
					unsigned shift_amt = (IEEErepBits - y +
							      leastZeroBit) %
							     nDims;
					*rotation = (shift_amt + 2 + strayBit) %
						    nDims;
		 ...
		 *
		 * Determine which of two points lies further along the Hilbert
		 curve
		 * Inputs:
		 ...
		 *  coord1:     Array of nDims nBytes-byte coordinates (or
		 doubles for ieee_cmp).
		 *  coord2:     Array of nDims nBytes-byte coordinates (or
		 doubles for ieee_cmp).
		 * Return value:
			...
			bitmask_t index, BitReader getBits)
		{
			...
			bitmask_t const nd1Ones =
				ones(bitmask_t, nDims) >>
				1; /* used in adjust_rotation macro */
			while (y-- > max) {
				bitmask_t reflection =
					getBits(nDims, nBytes, c1, y);
				bitmask_t diff = reflection ^
						 getBits(nDims, nBytes, c2, y);
				bits ^= reflection;
				bits = rotateRight(bits, rotation, nDims);
				if (diff) {
					unsigned d;
					diff = rotateRight(diff, rotation,
							   nDims);
					for (d = 1; d < nDims; d *= 2) {
						index ^= index >> d;
						bits ^= bits >> d;
					    ...
					    double const *c2)
		{
			...
			if (getIEEESignBits(nDims, c1) !=
			    getIEEESignBits(nDims, c2))
				max = 2047;
			else {
			...
			getIEEEinitValues(c1, max + 53, nDims, &rotation, &bits,
					  &index);
			return hilbert_cmp_work(nDims, 8, 64, max, max + 53,
						(char const *)c1,
						(char const *)c2, rotation,
						bits, index, getIEEEBits);
		}

		/*****************************************************************
		 * hilbert_box_vtx
		 *
		 * Determine the first or last vertex of a box to lie on a
		 * Hilbert curve Inputs: nDims:      Number of coordinates.
		 *  nBytes:     Number of bytes/coordinate.
		 *  nBits:      Number of bits/coordinate.
		 *  findMin:    Is it the least vertex sought?
		 *  coord1:     Array of nDims nBytes-byte coordinates - one
		 * corner of box coord2:     Array of nDims nBytes-byte
		 * coordinates - opposite corner Output: c1 and c2 modified to
		 * refer to selected corner value returned is log2 of size of
		 * largest power-of-two-aligned box that contains the selected
		 * corner and no other corners Assumptions: nBits <= (sizeof
		 * bitmask_t) * (bits_per_byte)
		 */

		static unsigned hilbert_box_vtx_work(
			unsigned nDims, unsigned nBytes, unsigned nBits,
			...
			bitmask_t index, BitReader getBits)
		{
			...
			while (y--) {
				...
				if (diff) {
					unsigned d;
					bitmask_t smear =
						rotateRight(diff, rotation,
							    nDims) >>
						1;
					bitmask_t digit =
						rotateRight(bits ^ reflection,
							    rotation, nDims);
					for (d = 1; d < nDims; d *= 2) {
						index ^= index >> d;
						digit ^= (digit >> d) & ~smear;
					...
					if ((index ^ y ^ findMin) & 1)
						digit ^= smear + 1;
					digit = rotateLeft(digit, rotation,
							   nDims) &
						diff;
					reflection ^= digit;
			...
			unsigned nDims, int findMin, double *c1, double *c2)
		{
			...
			return hilbert_box_vtx_work(nDims, 8, 64, findMin, max,
						    max + 53, (char *)c1,
						    (char *)c2, rotation, bits,
						    index, getIEEEBits);
		}
		 ...
		 * hilbert_box_pt
		 *
		 * Determine the first or last point of a box to lie on a
		 * Hilbert curve Inputs: nDims:      Number of coordinates.
		 *  nBytes:     Number of bytes/coordinate.
		 *  nBits:      Number of bits/coordinate.
		 *  findMin:    Is it the least vertex sought?
					       ...
					       void *c1, void *c2)
		{
			...
			return hilbert_box_pt_work(
				nDims, nBytes, nBits, findMin, 0, nBits,
						    ...
						    double *c1, double *c2)
		{
			...
			if (c1Signs != c2Signs) {
				...
				index = 1;
				hilbert_box_pt_work(
					nDims, 8, 64, findMin, IEEErepBits - 1,
					IEEErepBits, (char *)c1, (char *)c2,
		 ...
		 * hilbert_nextinbox
		 *
		 * Determine the first point of a box after or before a given
		 point to lie on
		 * a Hilbert curve
		 * Inputs:
		 *  nDims:      Number of coordinates.
		 ...
		 *  coord2:     Array of nDims nBytes-byte coordinates -
		 opposite corner
		 *  point:      Array of nDims nBytes-byte coordinates - lower
		 bound on point returned
		 *
		 * Output:
		      if returns 1:
		 *      c1 and c2 modified to refer to least point after "point"
		 in box else returns 0: arguments unchanged; "point" is beyond
		 the last point of the box
		 * Assumptions:
		 *      nBits <= (sizeof bitmask_t) * (bits_per_byte)
		 */
			...
			int findPrev, void *c1V, void *c2V, void const *ptV)
		{
			...
			char *c1 = (char *)c1V;
			char *c2 = (char *)c2V;
			char const *pt = (char const *)ptV;

			while (y-- > 0) {
				bitmask_t reflection =
					getIntBits(nDims, nBytes, pt, y);
				bitmask_t diff =
					reflection ^ /* planes that separate box
							and point */
					((getIntBits(nDims, nBytes, c1, y) &
					  ~fold1) |
					 valu1);

				if (diff)
				/* some coordinate planes separate point from
				   box or dividing box or both; smear the bits
				   of diff to reflect that after the first diff
				   dimension, they might as well all be diffing;
				   adjust the diff to reflect the fact that
				   diffed dimensions don't matter. */
				{
					/* compute (the complement of) a "digit"
					   in the integer index of this point */
					bitmask_t cornerdiff =
						(diff ^
						 reflection) ^ /* separate box
								  crnrs */
						((getIntBits(nDims, nBytes, c2,
							     y) &
						  ~fold2) |
						 valu2);
					bitmask_t separator =
						diff & ~cornerdiff;
					/* eventually, the most significant
					 * separating cutting plane */
					bitmask_t firstSeparator;
					/* bits less significant than the msb of
					   separator are irrelevant; for
					   convenience, call them all separators
					   too */
					bitmask_t rotSep = rotateRight(
						separator, rotation, nDims);
					/* compute the (complement of the) digit
					   of the hilbert code assoc with point
					 */
					bitmask_t digit =
						rotateRight(bits ^ reflection,
							    rotation, nDims);
					...
					separator = rotateLeft(rotSep, rotation,
							       nDims);
					rotSep -= rotSep >> 1;
					firstSeparator = rotateLeft(
						rotSep, rotation, nDims);
					/* forget about all the planes that
					   split the box, except those that are
					   more significant than the most
					   significant separator. */
					cornerdiff &= ~separator;

					if (cornerdiff && digit)
					/* some coordinate planes divide the
					   box.  Call the part of the box in the
					   same orthant as the point "here" and
					   the part of the box in the next (or
					   previous) orthant "there".  Remember
					   what the "there" orthant of the box
					   looks like in case it turns out that
					   the curve doesn't reenter the box
					   "here" after (before) passing thru
					   point.  Continue working with the
					   "here" part. If there is no "there"
					   there, skip it */
					{
						p_firstSeparator =
							digit & -digit;
						p_separator =
							2 * p_firstSeparator -
							1;
						p_separator = rotateLeft(
							p_separator, rotation,
							nDims);
						p_firstSeparator = rotateLeft(
							p_firstSeparator,
							rotation, nDims);
						p_cornerdiff =
							cornerdiff &
					...
					if (firstSeparator)
					/* we have completely separated the
					   point from a part of the box ahead of
					   it on the curve; almost done */
					{
						unsigned byteId =
							whichByte(nBytes, y);
		...
		/* LICENSE
		 *
		 * This software is copyrighted by Rice University.  It may be
		 ...
		 * them as he desires.
		 *
		 * You may include this software in a program or other software
		 * product, but must display the notice:
		 *
		 ...
		 * the date of modification, and the reason for such
		 * modification.
		 */
		...
		/* Revision history:
		   ...
		   Dec 1998: Fixed bug in hilbert_c2i that allowed a shift by
		   number of bits in bitmask to vaporize index, in last bit of
		   the function.  Implemented hilbert_incr.

		   August 1999: Added argument to hilbert_nextinbox so that you
		   can, optionally, find the previous point along the curve to
		   intersect the box, rather than the next point.

		   Nov 1999: Defined fast bit-transpose function (fast, at
		   least, if the number of bits is large), and reimplemented i2c
		   and c2i in terms of it.  Collapsed loops in hilbert_cmp, with
		   the intention of reusing the cmp code to compare more general
		   bitstreams.

		   Feb 2000: Implemented almost all the floating point versions
		   of cmp, etc, so that coordinates expressed in terms of
		   double-precision IEEE floating point can be ordered.  Still
		   have to do next-in-box, though.

		   Oct 2001: Learned that some arbitrary coding choices caused
		   some routines to fail in one dimension, and changed those
		   choices.
		...
		*/
		...
		/* What remains is test code that won't be compiled unless you
		   define the TEST_HILBERT preprocessor symbol */
...
#define maxDim (8 * sizeof(bitmask_t))
			...
			for (;;) {
				printf("Enter nDims, nBits, nPrints, "
				       "orderCheck: ");
				scanf("%d", &nDims);
				if (nDims == 0)
					break;
				...
				if (nDims * nBits > 8 * sizeof(r)) {
					printf("Product of nDims and nBits not "
					       "exceed %d.\n",
					       8 * sizeof(r));
					break;
				}
				...
				for (r = 0; r < nPrints; ++r) {
					...
					for (r1 = 0; r1 < r; ++r1) {
						unsigned ans;
						hilbert_i2c(nDims, nBits, r1,
							    coord1);
						ans = hilbert_cmp(
							nDims, sizeof(coord[0]),
							nBits, coord, coord1);
						if (ans != 1) {
							int width =
								(nDims * nBits +
					...
					if (hilbert_cmp(nDims, sizeof(coord[0]),
							nBits, coord,
							coord1) != 0)
						printf("cmp r = 0x%0*x; r1 = "
						       "0x%0*x\n",
						       (nDims * nBits + 3) / 4,
						       (unsigned)r,
						       (nDims * nBits + 3) / 4,
...
#define maxDim (8 * sizeof(bitmask_t))
		...
		{
			return hilbert_cmp(g_nDims, sizeof(unsigned),
					   8 * sizeof(unsigned), c1p, c2p);
		}

		int main()
		{
			unsigned corner0[maxDim], corner1[maxDim];
			unsigned cornerlo[maxDim], cornerhi[maxDim],
				work[maxDim];
			...
			hilbert_box_vtx(nDims, sizeof(unsigned),
					8 * sizeof(unsigned), 1, cornerlo,
					work);
			...
			array = (array_t *)malloc(maxDim * sizeof(unsigned)
						  << nDims);
			for (k = 0; k < (1 << nDims); ++k) {
				unsigned j;
			...
			g_nDims = nDims;
			qsort(array, (1 << nDims), maxDim * sizeof(unsigned),
			      cmp);
...
#define maxDim (8 * sizeof(bitmask_t))
		...
		int main()
		{
			...
			for (i = 0; i < 10000; ++i) {
				...
				array = (array_t *)malloc(maxDim * sizeof(key_t)
							  << nDims);
				for (k = 0; k < (1 << nDims); ++k) {
					unsigned j;
...
#define maxDim (8 * sizeof(bitmask_t))
		...
		int main()
		{
			...
			printf("Enter other point (%d coordinates, strictly "
			       "greater): ",
			       nDims);
			for (k = 0; k < nDims; ++k) {
				unsigned diff;
			...
			hilbert_box_pt(nDims, sizeof(unsigned),
				       8 * sizeof(unsigned), 1, pointlo, work);
			...
			hilbert_box_pt(nDims, sizeof(unsigned),
				       8 * sizeof(unsigned), 0, work, pointhi);
			...
			g_nDims = nDims;
			qsort(array, outvolume - involume,
			      maxDim * sizeof(unsigned), cmp);

			printf("Result of sort\n");
			for (k = 0; k < outvolume - involume;
			     k += outvolume - involume - 1) {
				unsigned j;
				unsigned *eltk = &array[k][0];
				for (j = 0; j < nDims; ++j)
					printf("%4u", eltk[j]);
...
#define nPts 1000000
			...
			for (i = 0; i < nPts; ++i) {
				...
				if (hilbert_ieee_cmp(nDims, pt1, pt2) < 0) {
					if (hilbert_ieee_cmp(nDims, pt1,
							     pointlo) < 0)
						memcpy(pointlo, pt1,
						       maxDim * sizeof(double));
					if (hilbert_ieee_cmp(nDims, pointhi,
							     pt2) < 0)
						memcpy(pointhi, pt2,
						       maxDim * sizeof(double));
				} else {
					if (hilbert_ieee_cmp(nDims, pt2,
							     pointlo) < 0)
						memcpy(pointlo, pt2,
						       maxDim * sizeof(double));
					if (hilbert_ieee_cmp(nDims, pointhi,
...
#include <stdio.h>
		...
		int main()
		{
			...
			stat = hilbert_nextinbox(nDims, nBytes, 8 * nBytes,
						 findPrev, c1, c2, pt);
