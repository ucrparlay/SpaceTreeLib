	      ...
	      kImbaRatio>::BatchInsert_(Slice A)
{
	...
	this->root_ =
		BatchInsertRecursive(T, A, B.cut(0, A.size()), this->tree_box_);
						       ...
						       &sums)
{
	...
	SerialSplitSkeleton(T, In.cut(0, split_iter - In.begin()), dim + 1,
			    2 * idx, sums);
	SerialSplitSkeleton(T, In.cut(split_iter - In.begin(), In.size()),
			    dim + 1, 2 * idx + 1, sums);
	return;
}
						 ...
						 Box const &box)
{
	...
	return IT.template UpdateInnerTree<InnerTree::kUpdatePointer>(
		tree_nodes);
}
} // namespace psi
