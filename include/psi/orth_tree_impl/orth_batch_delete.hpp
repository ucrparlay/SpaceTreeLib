	      ...
	      kImbaRatio>::BatchDelete_(Slice A)
{
	...
	this->root_ = BatchDeleteRecursive(
						 ...
						 Box const &box, bool has_tomb)
{
	...
	[[maybe_unused]] Node *new_node =
		IT.template UpdateInnerTree<InnerTree::kTagRebuildNode>(
			tree_nodes);
	...
	parlay::parallel_for(0, IT.tags_num, [&](size_t i) {
      ...
      BT::template DeleteTreeRecursive<Leaf, Interior, true>(
          IT.tags[IT.rev_tag[i]].first);
			...
			IT.tags[IT.rev_tag[i]].first =
				BT::template RebuildSingleTree<Leaf, Interior,
							       false>(
					IT.tags[IT.rev_tag[i]].first,
					IT.GetBoxByRegionIdx(IT.rev_tag[i],
							     box));
		}
	}); // PERF: allow the parlay decide the granularity to accelerate the
	    // small tree rebuild

	return IT.template UpdateInnerTree<InnerTree::kPostDelUpdate>(
		tree_nodes);
}
