...
struct OrthTree<Point, SplitRule, LeafAugType, InteriorAugType, kMD, kSkHeight,
	...
	constexpr auto GetSubTreeNum() const
	{
		return BaseNode::GetRegions();
	}
	...
	void GenerateHyperPlaneSeq(HyperPlaneSeq &hyper_seq, auto idx,
				   auto deep)
	{
		if (idx >= BaseNode::GetRegions()) {
			return;
	...
	BoxSeq ComputeSubregions(Box const &box)
	{
		...
		ComputeSubregionsRec(parlay::make_slice(box_seq), this->split,
				     0);
	...
	template <typename Box>
	static Box GetBoxByRegionId(BucketType id, ST const &split,
				    Box const &box)
	{
		Box new_box(box);
