...
size_t
BaseTree<Point, DerivedTree, kSkHeight, kImbaRatio>::RangeCountRectangleLeaf(
	...
	BucketType idx, RangeQueryLogger &logger)
{
	...
	auto recurse = [&query_box, &logger](Node *Ts, Box const &box,
		...
		if (!BoxIntersectBox(box, query_box)) {
			...
			counter = static_cast<Interior *>(Ts)->MergeSize(
				next_idx);
		} else {
			counter = RangeCountRectangle<Leaf, Interior>(
						     ...
						     RangeQueryLogger &logger)
{
	...
	auto recurse = [&query_box, &s, &Out,
			&logger](Node *Ts, Box const &box, DimsType next_dim,
		...
		if (!BoxIntersectBox(box, query_box)) {
			...
			RangeQuerySerialRecursive<Leaf, Interior>(
				Ts, Out, s, query_box, box, next_dim, next_idx,
				logger);
			return;
		}
	};
	...
	recurse(reach_leaf ? TI->tree_nodes[idx - Interior::GetRegions()] : T,
		box_cut.GetFirstBoxCut(), (dim + 1) % kDim,
		reach_leaf ? 1 : idx);

	// NOTE: visit right
	idx |= 1;
	recurse(reach_leaf ? TI->tree_nodes[idx - Interior::GetRegions()] : T,
		box_cut.GetSecondBoxCut(), (dim + 1) % kDim,
		reach_leaf ? 1 : idx);
