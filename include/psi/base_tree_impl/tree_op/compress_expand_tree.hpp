#ifndef PSI_BASE_TREE_IMPL_COMPRESS_EXPAND_TREE_HPP_
#define PSI_BASE_TREE_IMPL_COMPRESS_EXPAND_TREE_HPP_
...
template <IsBinaryNode BN, IsMultiNode MN>
Node *
BaseTree<Point, DerivedTree, kSkHeight, kImbaRatio>::Expand2Binary(Node *T)
...
void BaseTree<Point, DerivedTree, kSkHeight, kImbaRatio>::CompressBinaryNode(
	Node *bn_proto, typename MN::NodeArr &tree_nodes,
...
template <IsBinaryNode BN, IsMultiNode MN>
Node *
BaseTree<Point, DerivedTree, kSkHeight, kImbaRatio>::Compress2Multi(Node *T)
	requires std::same_as<typename BN::ST, typename MN::ST::value_type>
{
	if (T->is_leaf) {
		return T;
	...
	if (BN::template TestDepth<BN>(T, 0, MN::GetLevels())) {
		...
		auto o = AllocInteriorNode<MN>(tree_nodes, split,
					       typename MN::AT());
		...
		auto o = AllocInteriorNode<BN>(L, R, bn->split,
					       typename BN::AT());
