							      ...
							      Box const &box)
{
	...
	assert(!TI->GetParallelFlagIniStatus()); // NOTE: ensure that
						 // uninitialized force
						 // parallelism
	if constexpr (IsBinaryNode<Interior> &&
		      !HasBox<typename Interior::AT>) { // NOTE: use hyperplane
							// as splitter
		Box lbox(box), rbox(box);
		lbox.second.pnt[TI->split.second] = TI->split.first;
		rbox.first.pnt[TI->split.second] = TI->split.first;
		Box const left_return_box =
			CheckBox<Leaf, Interior>(TI->left, lbox);
		Box const right_return_box =
			CheckBox<Leaf, Interior>(TI->right, rbox);
		...
		static_assert(IsBinaryNode<Interior> || IsMultiNode<Interior>,
			      "CheckBox supports only binary and multi-way "
			      "interior nodes");
		return GetEmptyBox();
	}
}
	...
	Node *T, typename Interior::CircleType const &level_cover_circle)
{
	...
	for (size_t i = 0; i < TI->tree_nodes.size(); i++) {
		auto next_circle = TreeCircle{
			TI->split[i],
			static_cast<DepthType>(TI->GetCoverCircle().level - 1)};
		return_points_seq[i] = CheckCover<Leaf, Interior>(
...
size_t BaseTree<Point, DerivedTree, kSkHeight, kImbaRatio>::CheckSize(Node *T)
{
	...
	if constexpr (IsBinaryNode<Interior>) {
		...
		size_t l, r;
		parlay::par_do(
			[&l, &TI] { l = CheckSize<Leaf, Interior>(TI->left); },
			[&r, &TI] {
	      ...
	      kImbaRatio>::CheckTreeSameSequential(Node *T, int dim)
{
	...
	if constexpr (IsBinaryNode<Interior>) {
		...
		parlay::par_do_if(
			T->size > 1000,
			[&]() {
				CheckTreeSameSequential<Leaf, Interior>(
					TI->left, dim);
			},
			[&]() {
				CheckTreeSameSequential<Leaf, Interior>(
...
void BaseTree<Point, DerivedTree, kSkHeight, kImbaRatio>::Validate()
{
	...
	if constexpr (IsBinaryNode<Interior> || IsMultiNode<Interior>) {
		if (LegalBox(CheckBox<Leaf, Interior>(this->root_,
						      this->tree_box_))) {
		...
		if constexpr (IsRotateDimSplit<
				      typename SplitRule::DimRuleType> &&
...
template <typename Leaf, typename Interior>
size_t
BaseTree<Point, DerivedTree, kSkHeight, kImbaRatio>::CountTreeNodesNum(Node *T)
{
	if (T->is_leaf) {
		return 1;
	}
	...
	if constexpr (IsBinaryNode<Interior>) {
		...
		parlay::par_do(
			...
			[&]() {
				r = CountTreeNodesNum<Leaf, Interior>(
					TI->right);
			});
		return l + r + 1;
	} else {
		...
		for (int i = 0; i < TI->tree_nodes.size(); i++) {
			sum += CountTreeNodesNum<Leaf, Interior>(
				TI->tree_nodes[i]);
		}
		return sum + 1;
	}
	...
	Node *T, size_t deep, size_t &idx, parlay::sequence<size_t> &heights)
{
	...
	Interior *TI = static_cast<Interior *>(T);
	if constexpr (IsBinaryNode<Interior>) {
		CountTreeHeights<Leaf, Interior>(TI->left, deep + 1, idx,
						 heights);
		CountTreeHeights<Leaf, Interior>(TI->right, deep + 1, idx,
						 heights);
	} else if constexpr (IsMultiNode<Interior>) {
		for (size_t i = 0; i < TI->tree_nodes.size(); i++) {
			CountTreeHeights<Leaf, Interior>(
