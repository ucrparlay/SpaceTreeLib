...
public:
	using BT =
		BaseTree<Point, PTree<Point, SplitRule, kSkHeight, kImbaRatio>,
			 kSkHeight, kImbaRatio>;
	...
	struct CpamEntry {
		...
		using key_entry_pointer =
			std::pair<typename Point::AT, entry_t *>;
		...
		{
			return BT::GetEmptyBox();
		}

		// WARN: this invoke implicity conversion from Coords to
		// BasicPoint
