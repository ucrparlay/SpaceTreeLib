				      ...
				      BoxSeq &box_seq, Box const &box)
{
	...
	DivideRotate(In.cut(n / 2, n), pivots, cutting_dim, 2 * idx + 1,
		     box_seq, box_cut.GetSecondBoxCut());
	return;
}
					       ...
					       DimsType dim, Box const &box)
{
	...
	assert(std::ranges::all_of(In.begin(), split_iter, [&](Point &p) {
		return Num::Lt(p.pnt[split.value().second],
			       split.value().first);
	}));
	assert(std::ranges::all_of(split_iter, In.end(), [&](Point &p) {
		return Num::Geq(p.pnt[split.value().second],
				split.value().first);
	}));
					 ...
					 Box const &bx)
{
	...
	return BT::template BuildInnerTree<Leaf, Interior>(1, pivots,
							   tree_nodes);
}
