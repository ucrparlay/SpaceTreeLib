...
auto kd_tree<Traits>::knn(point_type const &q,
...
template <typename Traits>
template <typename Range>
size_t kd_tree<Traits>::flatten(Range &&out) const
{
...
template <typename Traits>
auto kd_tree<Traits>::range_count(box_type const &bx) const
{
...
template <typename Traits>
template <typename Range>
auto kd_tree<Traits>::range_query(box_type const &query_box, Range &&out) const
{
	range_query_logger logger;
	size_t s = 0;
...
template <typename Traits>
constexpr void kd_tree<Traits>::delete_tree()
{
