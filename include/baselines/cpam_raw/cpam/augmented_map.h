...
public:
	...
	template <class F>
	static M aug_filter_mid(M m, F const &f)
	{
	...
	template <typename F, typename Out>
	static void range_report_filter2(M &m, F const &f, int64_t &cnt,
					 Out &out)
	{
	...
	static void knn_filter(M &m, F const &f, const F2 &f2, size_t &k,
			       Out &out, size_t &vis_leaf)
	{
	...
	A aug_val2(node *t) const
	{
		return Tree::aug_val(t);
	}
	...
	static M multi_delete(M m, parlay::sequence<E> const &SS)
	{ //  added interface for multi delete
	...
	template <class outItter>
	static void values(M m, outItter out)
	{
		Map::values(std::move(m), out);
	} // values added
