...
struct gc {
	...
	static void reserve(size_t n, bool randomize = false)
	{
		...
		std::cout << "Reserving.... " << (n / B)
			  << " nodes with size = " << sizeof(regular_node)
			  << std::endl;
		// std::cout << "Reserving.... " << (n/B) << " complex nodes
		// with size = " << sizeof(typename Node::complex_bytes) <<
		// std::endl;
	}
	static void finish()
	{
	...
	// copies node with entry and children, incrementing children's ref
	// counts does not update
	...
	{
		if (!t)
			std::cout << "copy if needed fail" << std::endl;
		node *res = t;
		if (Node::ref_cnt(t) > 1) {
			res = copy(t);
	...
	struct ptr {
		...
		// extra_ptr = true if there is an implicit extra pointer to
		// this node, but we don't want to bump the ref-cnt to encode
		// this explicitly. Instead, we store a bit to remember that
		// there's an extra pointer.
		ptr(node *a, bool extra_ptr = false) : p(set_flag(a, extra_ptr))
		{
			// cout << "construct: " << a << ", " << extra_ptr <<
			// endl;
		}
		...
		static node *set_flag(node *x, bool f)
		{
			return (node *)(((size_t)x) + f);
		}
		static bool has_flag(node *x)
		{
			return ((size_t)x) & 1;
		}
		static node *strip_flag(node *x)
		{
