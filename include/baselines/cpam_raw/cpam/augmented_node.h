...
struct aug_node
    : public basic_node<
          balance, std::pair<typename Entry::entry_t, typename Entry::aug_t>,
  ...
  using basic =
      basic_node<balance, std::pair<ET, AT>, null_encoder, kBlockSize>;
  ...
  // mid-order traversal
  template <typename F>
  static void iterate_seq_mid(node* a, F const& f) {
    if (!a) return;
  ...
  static void shuffle_leaf_points(node* b) {
    ...
    uint8_t* data_start = ((uint8_t*)c) + sizeof(aug_compressed_node);
    return AugEntryEncoder::shuffle(data_start, c->s);
  }
