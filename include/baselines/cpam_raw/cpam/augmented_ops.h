...
struct augmented_ops : Map {
			...
			Logger &logger)
	{
		...
		if (Map::is_compressed(b)) { // leaf node
			...
			Map::template iterate_seq<decltype(f_filter)>(b,
								      f_filter);
			return;
		}
		...
		if (!bq.full() || r < bq.top_value()) {
			bq.insert(std::make_pair(
				std::ref(std::get<1>(rb->entry.first)), r));
		}
	...
	static void knn_filter(node *b, F const &f, const F2 &f2, size_t &k,
				    ...
				    size_t granularity = kNodeLimit)
	{
		...
		//   [&]() {return aug_filter_mid(std::move(lc), f,
		//   granularity);},
		//   [&]() {return aug_filter_mid(std::move(rc), f,
		//   granularity);});
