...
struct red_black_tree {
	...
	struct balance : Node {
		...
		static bool is_balanced(node *t)
		{
			...
			if (!t)
				return true;
			bool f = true;
			if (!((t->color == BLACK) || (color(t->lc) == BLACK &&
						      color(t->rc) == BLACK))) {
				std::cout << "bad color" << std::endl;
				std::cout << color(t->lc) << " " << color(t)
					  << " " << color(t->rc) << std::endl;
				f = false;
			}
		...
		static node *node_join(node *t1, node *t2, node *k)
		{
			...
			if (height(t1) > height(t2)) {
				...
				if (t->color == BLACK && color(t->rc) == RED &&
				    color(t->rc->rc) == RED) {
					t->rc->rc->color = BLACK;
					t->rc->rc->height++;
			...
			if (height(t2) > height(t1)) {
				...
				if (t->color == BLACK && color(t->lc) == RED &&
				    color(t->lc->lc) == RED) {
					t->lc->lc->color = BLACK;
					t->lc->lc->height++;
	...
	private:
		...
		{
			// if (!t1) {
			// if (!t2) std::cout << "null and null" << std::endl;
			// else { std::cout << "null and non-null" << std::endl;
			// std::cout << height(t1) << " " << height(t2) <<
			// std::endl;
			// }
			// }
			if (height(t1) == height(t2) && color(t1) == BLACK)
				return balanced_join(t1, t2, k, RED);
			// if (!t1) std::std::cout << "right_join fail" <<
			// std::std::endl;
		...
		{
			// if (!t2) {
			// if (!t1) std::cout << "null and null" << std::endl;
			// else { std::cout << "null and non-null" << std::endl;
			// std::cout << height(t1) << " " << height(t2) <<
			// std::endl;
			// }
			...
			if (height(t1) == height(t2) && color(t2) == BLACK)
				return balanced_join(t1, t2, k, RED);
			// if (!t2) std::std::cout << "left_join fail" <<
			// std::std::endl;
