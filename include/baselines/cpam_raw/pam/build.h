...
struct build {
	...
	template <class Seq>
	static parlay::sequence<ET> sort_remove_duplicates(Seq const &A)
	{ // ?? const
		if (A.size() == 0)
	...
	// // this version does the sort inplace if passed as an rvalue
	// reference template <class Seq> static parlay::sequence<ET>
	...
	//   } else {
	//     parlay::internal::sample_sort_inplace(parlay::make_slice(A),
	//     less); auto g = [&] (size_t i) {return (i==0) || less(A[i-1],
	//     A[i]); }; auto C = parlay::pack(A,
	//     parlay::delayed_seq<bool>(A.size(),g)); return C;
	//   }
	// }
	...
	//   parlay::internal::sample_sort_inplace(parlay::make_slice(A,A+n),
	//   lessE);
