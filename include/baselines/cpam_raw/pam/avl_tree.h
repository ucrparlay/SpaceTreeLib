...
struct avl_tree {
	...
	struct balance : Node {
		...
		using t_utils =
			balance_utils<balance>; // bulk of work done here
		friend t_utils;
		...
		{
			// return !t || !(is_left_heavy(t->lc,t->rc) ||
			// is_left_heavy(t->rc,t->lc));
			return !t || !((is_left_heavy(t->lc, t->rc) ||
					is_left_heavy(t->rc, t->lc)));
		}
	...
	private:
		...
		static inline bool is_single_rotation(node const *t,
						      bool const dir)
		{
		...
		// static inline bool is_single_rotation(const node* t, const
		// bool dir) {
		// return dir ? (height(t->lc) > height(t->rc)) : (height(t->lc)
		// < height(t->rc));
		// }
	};
};
