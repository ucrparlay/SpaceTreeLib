...
public:
	...
	template <class Func>
	void iterate_aug(Func const &f)
	{
		Tree::iterate_aug(root, f);
	}

	// ??
	// static M multi_insert(M m, parlay::sequence<E> &&SS, bool sequential)
	// { // ?? should it be &
	//  return Map::multi_insert_xx(std::move(m), std::move(SS),
	//  sequential);}
	...
	static M multi_insert_reduce(M m, parlay::sequence<pair<K, Val>> S,
				     Reduce g)
	{ // ?? should it be &
	...
	{
		return to_aug(Map::join2(std::move(a), std::move(b)));
	}
	...
	template <class Ma, class F>
	static M map_filter(Ma a, F const &f)
	{
		return to_aug(Map::map_filter(a, f));
	}
	template <class F>
	static bool if_exist(M m, F const &f)
	{
		return Map::if_exist(m, f);
	}
	template <class Ma, class F>
	...
	template <class F>
	static void foreach_index(M const &m, F f, size_t start = 0,
				  size_t granularity = utils::node_limit)
	{
		Map::foreach_index(m, f, start, granularity);
	}
	template <class F>
	static void foreach_index_2(M const &m, F f, size_t start = 0,
				    size_t granularity = utils::node_limit)
	{
		Map::foreach_index(m, f, start, granularity);
	}
