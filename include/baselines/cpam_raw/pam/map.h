...
public:
	...
	{ //, bool seq_inplace = false) {
		M empty = M();
		root = multi_insert(empty, parlay::slice<E *, E *>(s, e))
			       .get_root();
	}
	...
	{ //, bool seq_inplace = false) {
		M empty = M();
		root = multi_insert_combine(empty,
	...
	{
		M empty = M();
		root = multi_insert_combine(empty, parlay::make_slice(S), f)
			       .get_root();
	}
	...
	// api)
	template <class F>
	static void foreach_index_2(M const &m, F const &f, size_t start = 0,
				    size_t granularity = utils::node_limit)
	{
		Tree::foreach_index(m.root, start, f, granularity, true);
	}
	...
	static bool foreach_cond_par(M const &m, F const &f, C const &c)
	{
		return Tree::foreach_cond(m.root, f);
	}
	...
	template <class F>
	static void foreach_seq(M m, F const &f)
	{
	...
	static parlay::sequence<OT>
	to_seq(M m, F const &f, size_t granularity = utils::node_limit)
	{
	...
	static parlay::sequence<E>
	entries(M m, size_t granularity = utils::node_limit)
	{
	...
	static parlay::sequence<K> keys(M m,
					size_t granularity = utils::node_limit)
	{
	...
	// elements.
	template <class F>
	static M filter(M m, F const &f, size_t granularity = utils::node_limit)
	{
	...
	template <class F>
	static bool if_exist(M m, F const &f)
	{
	...
	{
		return M(Tree::multi_delete_sorted(m.get_root(), SS.begin(),
						   SS.size()));
	}
	...
	{
		return M(Tree::multi_update_sorted(m.get_root(), SS.data(),
						   SS.size(), f));
	}
	...
	static M multi_update(M m, Seq const &SS, Bin_Op f)
	{
		...
		using BD = build<e_type>;
		parlay::sequence<pair<K, VV>> B =
			BD::sort_remove_duplicates(SS);
	...
	static V *multi_find(M m, Seq const &SS)
	{
		...
		V *ret = new V[B.size()];
		Tree::multi_find_sorted(m.get_root(), B.data(), B.size(), ret,
					0);
		return ret;
	}
	...
	V find(K const &key, V defaultv) const
	{
		...
		return (a != NULL) ? Entry::get_val(Tree::get_entry(a))
				   : defaultv;
	}
	...
	maybe_E next(K const &key) const
	{
		return node_to_entry(Tree::next(root, key));
	}

	maybe_E previous(K const &key) const
	{
	...
	static typename Reduce::T
	range_number_mr(M &a, K kl, size_t r, Map const &mp, Reduce const &rdc)
	{
	...
	template <class F>
	static void map_index(M &m, F const &f,
			      size_t granularity = utils::node_limit,
			      size_t start = 0)
	{
		Tree::foreach_index(m.root, start, f, granularity, true);
	...
	static typename R::T semi_map_reduce(M const &m, F const &f, R const &r,
	...
	template <class F>
	static void map_void(M &m, F const &f,
			     size_t granularity = utils::node_limit)
	{
		struct do_nothing {
			using T = bool;
	...
	template <class Ma, class F>
	static M map_filter(Ma const &a, F const &f,
			    size_t granularity = utils::node_limit)
	{
		return M(Tree::template map_filter<typename Ma::Tree>(
			a.root, f, granularity));
	}
	...
	{
		if (a != NULL)
			return maybe_V(Entry::get_val(Tree::get_entry(a)));
		else
			return maybe_V();
	}
	...
	{
		if (a != NULL)
			return maybe_E(Tree::get_entry(a));
		else
			return maybe_E();
	}
...
using pam_map =
	map_<map_full_entry<_Entry>,
	     typename Balance::template balance<
		     basic_node<typename Balance::data,
...
template <class _Entry, class Balance = weight_balanced_tree>
using pam_set = map_<set_full_entry<_Entry>,
		     typename Balance::template balance<basic_node<
			     typename Balance::data, typename _Entry::key_t>>>;
...
using pam_seq = map_<seq_full_entry<data>,
		     typename Balance::template balance<
			     basic_node<typename Balance::data, data>>>;
