...
struct gc {
	...
	static void decrement_recursive(node *t)
	{
		...
		if (decrement(t)) {
			utils::fork_no_result(
				Node::size(lsub) >= utils::node_limit,
	...
	//      recursive calls on left and right children passing copy as
	//      extra_ptr
	//    or inc_if(t->xc, copy) if no recursive call on child xc (x in
	//    {l,r})
	//      node* o = copy_if(t, copy, extra_ptr)
	//      return node_join(l, r, o) -- needs to set children of o, and
	//      update
