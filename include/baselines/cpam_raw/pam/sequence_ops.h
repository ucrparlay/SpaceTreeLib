...
struct sequence_ops : Tree {
	...
	template <typename F>
	static void foreach_index(node *a, size_t start, F const &f,
				  ...
				  bool extra_ptr = false)
	{
		...
		utils::fork_no_result(
			lsize >= granularity,
			[&]() {
				foreach_index(a->lc, start, f, granularity,
					      copy);
			},
			[&]() {
				foreach_index(a->rc, start + lsize + 1, f,
					      granularity, copy);
			});
		if (!extra_ptr)
			GC::decrement(a);
	...
	template <typename F>
	static void foreach_seq(node *a, F const &f, bool extra_ptr = false)
	{
		if (!a)
			return;
		bool copy = extra_ptr || (a->ref_cnt > 1);
	...
	template <class Func>
	static node *filter(node *b, Func const &f,
			    size_t granularity = utils::node_limit,
			    bool extra_ptr = false)
	{
		if (!b)
	...
	template <class Func>
	static bool if_exist(node *b, Func const &f, bool *indicator)
	{
		if (!b)
			return false;
		if (*indicator)
	...
	// 			    size_t start, size_t
	// grain=utils::node_limit) {
	//   if (!a) return identity;
	//   size_t lsize = Tree::size(a->lc);
	//   auto P = utils::fork<T>(Tree::size(a) >= grain,
	//     [&] () {return map_reduce_index(a->lc, m, r, identity, start,
	//     grain);},
	//     [&] () {return map_reduce_index(a->rc, m, r, identity, start +
	//     lsize + 1, grain);});
	...
	template <class R, class F>
	static typename R::T semi_map_reduce(node *b, F &f, R reduce,
					     size_t grain)
	{
		using T = typename R::T;
		if (Tree::size(b) >= grain) {
			T r, l;
			auto left = [&]() -> void {
				r = semi_map_reduce<R, F>(b->rc, f, reduce,
							  grain);
			};
			auto right = [&]() -> void {
				l = semi_map_reduce<R, F>(b->lc, f, reduce,
							  grain);
			};
			...
			// when going sequential, create identity and then
			// update it
