...
struct Point {
	...
	using bp_type = Point;
	Point(coords_type const &_pnt) : x(_pnt[0]), y(_pnt[1]), z(_pnt[2])
	{
	}
	...
	Point const max_coords(Point const &b) const
	{
		...
		p[2] = num_type::max(z, b[2]);
		return p;
	}
	// static consteval auto get_dim() { return 2; }
	...
	{
		// os << fixed << setprecision(6) << p.id << ": (" << p.x << ",
		// " << p.y <<
		// ", " << p.z << ")" << bitset<64>(p.morton_id); os << fixed <<
		// setprecision(6) << p.id << ": (" << p.x << ", " << p.y << ",
		// " << p.z <<
		// ")";
		os << fixed << setprecision(6)
		   << "Zvalue: " << bitset<64>(p.morton_id) << ", ID: " << p.id
		   << ", XYZ: (" << p.x << ", " << p.y << ", " << p.z << ")";
		return os;
	}
	...
	{
		/* 3D Morton Value Form: xyzxyz...xyz- the lowest bit is not
		 * used. */
		...
		// // Pun the x and y coordinates as integers: Just re-interpret
		// the bits. auto ix = static_cast<unsigned int>(x); auto iy =
		// static_cast<unsigned int>(y);

		// auto ret = 0ull;
		// for (auto i = 0; i < 32; i++) {
		//   ret |= ((ix & (1ull << i)) << (i + 1)) | ((iy & (1ull <<
		//   i)) << i);
		// }
		// return ret;
	}
	...
	long long mortonIndex() const
	{
		...
		// Since we're assuming 2s complement arithmetic (99.99% of
		// hardware today), we'll need to convert these raw
		// integer-punned floats into their corresponding integer
		// "indices".
		...
		// Now we have -FLT_MAX close to 0, and FLT_MAX close to
		// UINT_MAX, with everything else in-between.
		//
		// To make this easy, we'll work with x and y as 64-bit
		// integers.
		//
		long long xx = ix;
		long long yy = iy;
	...
	while (i < sorted_add.size() && j < sorted_remove.size()) {
		if (sorted_add[i].id <
			...
			insert_points.emplace_back(sorted_add[i++]);
			   ...
			   sorted_remove[j]
				   .id) { //	exist in both add and remove,
...
{
	cout << fixed << setprecision(6) << "[(" << mbr.first.x << ", "
	     << mbr.first.y << ", " << mbr.first.z << "), (" << mbr.second.x
	     << ", " << mbr.second.y << ", " << mbr.second.z << ")" << "]"
	     << endl;
}
...
bool point_in_mbr(Point &p, MBR &mbr)
{
	return mbr.first.x <= p.x && p.x <= mbr.second.x &&
	       ...
	       mbr.first.z <= p.z && p.z <= mbr.second.z;
}
...
int mbr_mbr_relation(MBR &small_mbr, MBR &large_mbr, size_t d = 3)
{
	...
	if (d == 2) {
		if (minc_x <= maxc_x && minc_y <= maxc_y) {
			if (minc_x == small_mbr.first.x &&
			    maxc_x == small_mbr.second.x &&
			    minc_y == small_mbr.first.y &&
			    maxc_y == small_mbr.second.y)
				return 1;
			return 0;
		}
		return -1;
...
{
	FT dx = max(max(mbr.first.x - p.x, (FT)0.0),
		    max(p.x - mbr.second.x, (FT)0.0));
	FT dy = max(max(mbr.first.y - p.y, (FT)0.0),
		    max(p.y - mbr.second.y, (FT)0.0));
	FT dz = max(max(mbr.first.z - p.z, (FT)0.0),
		    max(p.z - mbr.second.z, (FT)0.0));
...
{
	return dcmp(a.first.x - b.first.x) >= 0 &&
...
{
	return !(dcmp(a.second.x - b.first.x) < 0 ||
		 dcmp(a.first.x - b.second.x) > 0 ||
		 dcmp(a.second.y - b.first.y) < 0 ||
		 dcmp(a.first.y - b.second.y) > 0 ||
		 dcmp(a.second.z - b.first.z) < 0 ||
		 dcmp(a.first.z - b.second.z) > 0);
}

template <class MBR>
auto zy_mbr_relation(MBR &a, MBR &b)
{
	if (!box_intersect_box(a, b))
		return -1;
	if (box_within_box(a, b))
		return 1;
	return 0;
}
...
auto compute_cur_box3(bounding_box_type &cur_mbr, FT &x_prefix, FT &y_prefix,
		      FT &z_prefix, size_t b, size_t split_axis)
{
	...
	if (split_axis == 0) { //  cut x
		...
		l_box.second.z =
			min(z_prefix + split_value - ft_eps, l_box.second.z);
...
auto merge_pts(P &a, T &b, DIFF &ret_diff, bool reverse = false)
{
	...
	while (i < a.size() && j < b.size()) {
		...
		if (!flag) { // same point
			...
			// parlay::assign_uninitialized(removed[cnt_remove++],
			// a[i++]);
			ret_diff.remove_point(a[i], reverse);
			i++;
		} else { //  second smaller, in B not in A
			// parlay::assign_uninitialized(added[cnt_add++],
			// b[j++]);
...
auto merge_pts(P &a, T &b, F &f, DIFF &ret_diff, bool reverse = false)
{
	...
	while (i < a.size() && j < b.size()) {
		...
		if (!flag) { // same point
			...
			if (f(b[j])) {
				ret_diff.add_point(b[j], reverse);
				// parlay::assign_uninitialized(ret_diff.add[ret_diff.add_cnt++],
				// b[j]);
			}
			j++;
		}
...
auto get_sorted_points(pt_type &P, bool use_hilbert = false)
{
	...
	parlay::parallel_for(0, n, [&](int i) {
		P[i].morton_id = use_hilbert ? P[i].overlap_bits()
					     : P[i].interleave_bits();
		// unsigned long a = P[i].x, b = P[i].y, c = P[i].z;
		// P[i].morton_id = libmorton::m3D_e_for<unsigned long long,
		// unsigned long>(a, b, c);
	});
...
auto get_sorted_points_hilbert(pt_type &P, bool use_hilbert = true)
{
	...
	// static_cast<unsigned long long>(rhs.y)}; 	return hilbert_cmp(2,
...
auto count_duplicate_zvalues(pt_type &P)
{
	...
	for (auto const &pair : zvalue_map) {
		if (pair.second > 1) {
			// cout << "Number " << pair.first << " appears " <<
			// pair.second << " times." << endl;
			cnt++;
			total_cnt += pair.second;
		}
	}
	cout << "Total # of duplicate keys: " << cnt << ", " << total_cnt
	     << endl;
	return cnt;
}
...
auto diff_by_id(T &a, T &b)
{
	...
	while (i < a.size() && j < b.size()) {
		// cout << i << ", " << j << endl;
		if (sorted_a[i].id ==
		    sorted_b[j].id) { // same point, check coordinate
			if (sorted_a[i] ==
			    sorted_b[j]) { //  same coordinate, do nothing
				i++, j++;
			} else {
...
auto merge_by_id_with_conflict(pt_type &a, pt_type &b)
{
	...
	while (i < sorted_a.size() && j < sorted_b.size()) {
		if (sorted_a[i].id ==
			...
			if (!(sorted_a[i] ==
			      sorted_b[j])) { //  coordinate does not match,
					      //  conflict
