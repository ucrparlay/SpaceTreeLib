...
template <typename s_size_t = size_t, typename InIterator, typename OutIterator,
          typename Compare>
void sample_sort_inplace_(slice<InIterator, InIterator> In,
  ...
  if (n < QUICKSORT_THRESHOLD) {
    ...
    auto pivots = delayed_seq<value_type const&>(
        num_buckets - 1, [&](size_t i) -> value_type const& {
    ...
    sliced_for(n, block_size, [&](size_t i, size_t start, size_t end) {
      // The sample blocks are already sorted, so we don't need to sort them
      // again.
      if (i >= sample_blocks) {
        seq_sort_<uninitialized_relocate_tag>(
            In.cut(start, end), make_slice(Tmp).cut(start, end), less, false);
        get_bucket_counts(
            make_slice(Tmp).cut(start, end), make_slice(pivots),
            make_slice(counts).cut(i * num_buckets, (i + 1) * num_buckets),
            less);
      } else {
        get_bucket_counts(
            make_slice(In).cut(start, end), make_slice(pivots),
            make_slice(counts).cut(i * num_buckets, (i + 1) * num_buckets),
            less);
      }
    });
    ...
    parlay::uninitialized_relocate(sample_set.begin(), sample_set.end(),
                                   Tmp.begin());
...
void sample_sort_(slice<InIterator, InIterator> In,
  ...
  if (n < QUICKSORT_THRESHOLD) {
    ...
    counts[m] = 0;
    sliced_for(n, block_size, [&](size_t i, size_t start, size_t end) {
      seq_sort_<uninitialized_copy_tag>(
...
void sample_sort_inplace(slice<Iterator, Iterator> A, Compare const& less) {
  if (A.size() < (std::numeric_limits<unsigned int>::max)()) {
