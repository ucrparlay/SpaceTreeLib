...
//  // The block size to use for compressed trees. TODO: we may want to make
//  this
...
template <class V>
struct get_left {
  V operator()(V const& a, V const& b) const { return a; }
};

template <class V>
struct get_right {
  V operator()(V const& a, V const& b) const { return b; }
};
