...
struct balance_utils {
  ...
  static node* balanced_join(node* l, node* r, regular_node* e) {
    ...
    // 2.45B + 1
    // size_t upper_threshold = ceil(B * (static_cast<double>(1) / wb_ratio)) +
    // 1;
...
#endif
    auto go_reorder = [&](node* p) {
      auto c = Node::cast_to_compressed(p);
      uint8_t* data_start =
          (((uint8_t*)c) + sizeof(typename Node::aug_compressed_node));
      Node::reorder(c, data_start);
    };
