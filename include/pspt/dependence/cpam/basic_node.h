...
struct basic_node {
 public:
  ...
  static bool is_simplex(node* a) { return size(a) <= 2 * B; }
  ...
  static regular_node* generic_node(node* a) { return (regular_node*)a; }
  static node_size_t size(node* a) {
    return (a == NULL) ? 0 : generic_node(a)->s;
  }
    ...
    if (is_regular(t)) {
      ...
      if (decrement(t)) {
        utils::fork_no_result(
            size(lsub) >= kNodeLimit, [&]() { decrement_recursive(lsub); },
  ...
  static void print_node_info(node* a, std::string node_name) {
    ...
    //    if (is_regular(a)) {
    //      auto r = cast_to_regular(a);
    //      std::cout << "Node " << node_name << " is regular. Key = " <<
    //      get_entry(a).first << " Size = " << size(a) << " left child = " <<
    //      size(r->lc) << " right_child = " << size(r->rc) <<  std::endl;
    //    } else {
    //      auto c = cast_to_compressed(a);
    //      std::cout << "Node " << node_name << " is compressed. Size = " <<
    //      c->s << std::endl;
    //    }
  }
