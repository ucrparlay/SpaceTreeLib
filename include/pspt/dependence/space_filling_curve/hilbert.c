...
unsigned hilbert_box_pt(unsigned nDims, unsigned nBytes, unsigned nBits,
  ...
  bitmask_t const one = 1;
  bitmask_t bits = one << (nDims - 1);
  return hilbert_box_pt_work(nDims, nBytes, nBits, findMin, 0, nBits, (char*)c1,
                             (char*)c2, 0, bits, bits, getIntBits,
                             propogateIntBits);
}
...
#define maxDim (8 * sizeof(bitmask_t))
  ...
  for (;;) {
    ...
    if (nPrints > (1ULL << (nDims * nBits))) nPrints = 1ULL << (nDims * nBits);
