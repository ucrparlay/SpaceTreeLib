...
Thanks for AI, there is a generally good [wiki](https://deepwiki.com/ucrparlay/SpaceTreeLib) for this library, where you can get how things are operated, and get some start ideas if you want to contribute some code ☺️.
...
Everything start with the definition of the `Point` [(source code)](../include/psi/dependence/basic_point.h):
```{c++}
...
Next, as a spatial partition tree, we can choose how to split the space:
```{c++}
...
PSI also shipped a parallel data_generator for two distribution of points, namely, the `Uniform` and `Varden` (very skewed).
