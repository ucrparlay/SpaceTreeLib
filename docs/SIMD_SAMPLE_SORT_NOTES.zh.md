...
- `PTree` benchmark 使用新的 `32/32/64` 紧布局
- `USE_LIBMORTON_SIMD_ENCODE=ON`
- `USE_CPAM_HWY_MORTON_PREPARE=OFF`
- 也就是：packed `uint32_t` 点布局 + libmorton encode + SIMD sample sort
...
- 每次运行的第 1 条 `[CPAM build]` 是 warmup
- 后 3 条 `[CPAM build]` 是正式统计轮次
- 这里一共做了两次完整运行
- 下面的 `CPAM build` 数字，是把两次运行中后 3 轮、共 6 轮测量值合并后求平均
- `p_test` 最后一行打印的标量，则是两次完整运行最后一行的平均

| 路线 | fill / sort 阶段 | build 阶段 | CPAM total | build-only 平均值 |
| --- | ---: | ---: | ---: | ---: |
| 原版 (`legacy-interleaved-fill-sort`) | `1.521280s` (`sort+fill`) | `0.649931s` | `2.171211s` | `2.410875s` |
| SIMD (`simd-pretouch-lazybindcode-sort`) | `0.024925s touch + 1.383927s sort` | `0.650262s` | `2.059114s` | `2.355120s` |

这里 SIMD 路线里的 `touch` / `sort` / `build` 都是 6 次测量的平均：
...
- `[CPAM build] total`
  从 `2.171211s` 降到 `2.059114s`，快了 `0.112097s`
  (`1.054x`，提升约 `5.16%`)
- `build-only` 最后一行标量
  从 `2.410875s` 降到 `2.355120s`，快了 `0.055755s`
  (`1.024x`，提升约 `2.31%`)

### 为什么 `[CPAM build]` 能快 0.11 秒，但最后标量只快 0.056 秒
...
- SIMD: `2.355120 - 2.059114 = 0.296006s`
- Legacy: `2.410875 - 2.171211 = 0.239664s`

也就是说，SIMD 路线在 `CPAM core` 里省下来的时间，到了 end-to-end 只保留了一部分；剩下那部分会被：

- `PTree::Build(...)` 外层的 `aux` 分配与整段拷贝
- 一些模板/分配器相关的固定开销
- 以及几十毫秒量级的运行波动

一起稀释掉。

更准确地说，不是“SIMD 收尾比 OG 慢很多”，而是：

- `tree build` 本体几乎没差：`0.650262s` vs `0.649931s`
- 真正把优势拉小的，是 `CPAM build` 之外那层 end-to-end wrapper 开销
