...
class point3d {
   public:
    ...
    point minCoords(point b) { return point(min(x, b.x), min(y, b.y), min(z, b.z)); }
    point maxCoords(point b) { return point(max(x, b.x), max(y, b.y), max(z, b.z)); }
    coord& operator[](int i) { return (i == 0) ? x : (i == 1) ? y : z; }
    ...
    bool outOfBox(point pt, coord hsize) {
        return ((x - hsize > pt.x) || (x + hsize < pt.x) || (y - hsize > pt.y) || (y + hsize < pt.y) ||
...
template<class coord>
inline bool inCircle(point2d<coord> a, point2d<coord> b, point2d<coord> c, point2d<coord> d) {
...
template<class coord>
inline double inCircleNormalized(point2d<coord> a, point2d<coord> b, point2d<coord> c, point2d<coord> d) {
    vector3d<coord> ad = onParabola(a - d);
    vector3d<coord> bd = onParabola(b - d);
    vector3d<coord> cd = onParabola(c - d);
...
struct triangles {
    ...
    triangles() {}
    triangles(parlay::sequence<point> P, parlay::sequence<tri> T) : P(std::move(P)), T(std::move(T)) {}
};
...
inline coord minAngleCheck(point2d<coord> a, point2d<coord> b, point2d<coord> c, coord angle) {
    ...
    return (ba.dot(ca) / (lba * lca) > co || ca.dot(cb) / (lca * lcb) > co || -ba.dot(cb) / (lba * lcb) > co);
}
