...
void testCGALParallel(int Dim, int kLeaveWrap, parlay::sequence<Point>& wp, int N, int K, const int& rounds,
    ...
    if (queryType & (1 << 2)) {  //* range count
        ...
        for (int i = 0; i < 3; i++) {
            ...
            parlay::parallel_for(0, queryNum, [&](size_t i) {
                Point_d a(Dim, std::begin(queryBox[i].first.pnt), std::end(queryBox[i].first.pnt)),
                    b(Dim, std::begin(queryBox[i].second.pnt), std::end(queryBox[i].second.pnt));
        ...
        auto run_cgal_range_query = [&](int type) {
            ...
            tbb::parallel_for(
                tbb::blocked_range<std::size_t>(0, queryNum), [&](const tbb::blocked_range<std::size_t>& r) {
                    for (std::size_t s = r.begin(); s != r.end(); ++s) {
                        Point_d a(Dim, std::begin(queryBox[s].first.pnt), std::end(queryBox[s].first.pnt)),
                            b(Dim, std::begin(queryBox[s].second.pnt), std::end(queryBox[s].second.pnt));
                        Fuzzy_iso_box fib(a, b, 0.0);
...
int main(int argc, char* argv[]) {
    ...
    if (tag == -1) {
        ...
        testCGALParallel<Median_of_rectangle, Tree_Median, Neighbor_search_Median, PointType<Coord, 2>>(
            Dim, kLeaveWrap, pts, N, K, rounds, insertFile, tag, queryType);
    } else if (Dim == 3) {
        auto pts = parlay::tabulate(
