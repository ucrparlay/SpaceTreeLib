...
void runCGAL(auto const& wp, auto const& wi, auto const& query_points,
  ...
  for (int i = 0; i < 3; i++) {
    std::cout << kDim << wp[i].x << " " << wp[i].y << " " << wp[i].z
              << std::endl;
  }
  parlay::parallel_for(
      0, N,
      [&](size_t i) {
        parlay::sequence<typename Point::Coord> coords(kDim);
        coords[0] = wp[i].x;
        coords[1] = wp[i].y;
        coords[2] = wp[i].z;
        _points[i] =
            Point_d(kDim, std::begin(coords), (std::begin(coords) + kDim));
      },
      1000);
  ...
  tbb::parallel_for(tbb::blocked_range<std::size_t>(0, S),
                    [&](tbb::blocked_range<std::size_t> const& r) {
                      for (std::size_t s = r.begin(); s != r.end(); ++s) {
                        ...
                        parlay::sequence<typename Point::Coord> coords(kDim);
                        coords[0] = query_points[s].x;
                        coords[1] = query_points[s].y;
                        coords[2] = query_points[s].z;
                        Point_d query(kDim, std::begin(coords),
                                      std::begin(coords) + kDim);
...
void runPTreeParallel(auto const& wp, auto const& wi, Typename* kdknn,
  ...
  if (tag & (1 << 2)) {
    ...
    // BatchDiff<Point, Tree, kTestTime>(tree,
    // wp, 2, 0.5, 1.0); tree.template
    // Validate<typename Tree::Leaf, typename
    // Tree::Interior,
    //                        typename
    //                        Tree::SplitRuleType>();
    // assert(tree.GetSize() ==
    //        wp.size() -
    //        static_cast<size_t>(wp.size() *
...
int main(int argc, char* argv[]) {
  ...
  Wrapper::ApplyBaselines(tree_type, dim, split_type, params, run);
