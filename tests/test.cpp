...
void TestSpacialTree(const int& kDim, const parlay::sequence<Point>& wp,
    ...
    if (kTag >= 2) {
        if (kSummary) {
            const parlay::sequence<double> ratios = {0.0001, 0.001, 0.01, 0.1};
            for (int i = 0; i < ratios.size(); i++) {
                batchDelete<Point, Tree, kBatchDelete>(tree, wp, wi, kDim,
                                                       kRounds, 0, ratios[i]);
            }
        } else {
            batchDelete<Point, Tree, kBatchDelete>(tree, wp, wi, kDim, kRounds,
                                                   0, batchInsertRatio);
        }
    }
    ...
    if (kQueryType & (1 << 1)) {  // NOTE: batch NN query
        ...
        auto run_batch_knn = [&](const Points& pts, size_t batchSize) {
            ...
            parlay::copy(pts.cut(0, batchSize), newPts.cut(0, batchSize));
            kdknn = new Typename[batchSize];
            queryKNN<Point, Tree, true, true>(kDim, newPts, kRounds, tree,
                                              kdknn, K, true);
            delete[] kdknn;
        };
...
int main(int argc, char* argv[]) {
    ...
    auto run_test = [&]<class Wrapper>(Wrapper) {
        auto run = [&](auto dim_wrapper) {
            ...
            TestSpacialTree<Desc>(kDim, wp, wi, N, K, kRounds, insert_file_path,
                                  tag, kQueryType, kSummary);
        };
