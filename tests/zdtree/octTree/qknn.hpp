...
class q_intelementCompare {
  ...
  */
  bool operator()(std::pair<vtx*, double> const p1,
                  std::pair<vtx*, double> const p2) {
...
class qknn {
 private:
  ...
  // typedef std::priority_queue<q_intelement, parlay::sequence<q_intelement>,
  // q_intelementCompare>
  typedef std::priority_queue<q_intelement, std::vector<q_intelement>,
                              q_intelementCompare<vtx>>
      PQ;
  PQ pq;
  ...
  */
  ...
  q_intelement top() { return pq.top(); }

  double topdist(void) { return pq.top().second; }
  ...
  */
  void set_size(long unsigned int k) { K = k; }
