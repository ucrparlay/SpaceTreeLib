	     ...
	     int const query_type, int const K)
{
	...
	if (tag == 1) { // NOTE: only insert
		...
		parlay::parallel_for(0, wi.size(), [&](size_t j) {
			_points[j] = Point_d(kDim, std::begin(wi[j].pnt),
					     (std::begin(wi[j].pnt) + kDim));
		});
	...
	tbb::parallel_for(
		...
		[&](tbb::blocked_range<std::size_t> const &r) {
			for (std::size_t s = r.begin(); s != r.end(); ++s) {
				...
				Point_d query(
					kDim, std::begin(query_points[s].pnt),
		   ...
		   int const K, int const tag, int const rounds)
{
	...
	if (tag & (1 << 0)) {
		...
		tree.template Validate<typename Tree::Leaf,
	...
	if (tag & (1 << 2)) {
		BatchDiff<Point, Tree, kTestTime>(tree, wp, 2,
	...
	auto knn_checker = [&](Points const &wp, Points const &wi,
		...
		queryKNN<Point>(kDim, query_points, rounds, tree, kdknn, K,
				true);
		std::cout << "run cgal\n" << std::flush;
	...
	if (tag & (1 << 3)) { // incre insert
		...
		for (auto rat : ratios) {
			BatchInsertByStep<Point, Tree, true>(tree, wp, rounds,
							     rat);
			std::cout << "root size: " << tree.GetRoot()->size
				  << std::endl;
		}
		tree.template Validate<typename Tree::Leaf,
				       typename Tree::Interior,
	...
	if (tag & (1 << 4)) { // incre delete
		parlay::sequence<double> const ratios = {0.001};
		for (auto rat : ratios) {
			BatchDeleteByStep<Point, Tree, true>(tree, wp, rounds,
							     rat);
		}
		tree.template Validate<typename Tree::Leaf,
				       typename Tree::Interior,
	...
	if (tag == (1 << 0)) {
		...
		new_wp = wp.subseq(
			...
			wp.size());
	} else if (tag & (1 << 2)) { // WARN: this assume before we run diff,
				     // the tree should conation wp, e.g., just
				     // after build or build->insert->delete
	...
	if (query_type & (1 << 1)) { // NOTE: range count
		for (auto range_query_type : {0, 1, 2}) {
			...
			// tree.Flatten(new_wp);
			RangeCount<Point>(new_wp, tree, rounds, query_num,
