...
void runKDParallel(auto const &wp, auto const &wi, scalar_type *kdknn,
		   ...
		   int const K, int const tag, int const rounds)
{
	...
	if (tag & (1 << 3)) { // incre insert
		...
		for (auto rat : ratios) {
			batch_insert_by_step<Point, Tree, true>(tree, wp,
								rounds, rat);
			std::cout << "root size: " << tree.get_root()->size
				  << std::endl;
		}
		tree.template validate<typename Tree::leaf_type,
				       typename Tree::interior_type,
	...
	if (query_type & (1 << 1)) { // NOTE: range count
		for (auto range_query_type : {0, 1, 2}) {
			// points_type new_wp(tree.get_root()->size);
			// tree.flatten(new_wp);
...
int main(int argc, char *argv[])
{
	...
	wrapper::apply_orthogonal(tree_type, dim, split_type, params, run);
