...
inline int
less_msb( int x, int y ) {
  return x < y && x < ( x ^ y );
}

int
cmp_shuffle( Point *p, Point *q ) {
  int j, k, x, y;
  for ( j = k = x = 0; k < d; k++ )
...
float
dist_sq_to_box( Point q, Point p1, Point p2 ) {
  int i, j, x, y;
  float z;
  for ( j = x = 0; j < d; j++ )
...
void
SSS_query0( Point P[], int n, Point q ) {
  if ( n == 0 ) return;
  check_dist( P[n / 2], q );
...
Point
SSS_query( Point P[], int n, Point q ) {
  r_sq = FLT_MAX;
  SSS_query0( P, n, q );
  return ans;
}
