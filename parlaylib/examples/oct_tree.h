...
struct interior : node {
  ...
  interior(node* left, node* right) :
      node{false, left->size + right->size, bound_box(left->bounds, right->bounds), nullptr}, left(left), right(right) {
    left->parent = this;
    right->parent = this;
  }
};
...
node* build_recursive(slice P, int bit, long base_size) {
  ...
  if (bit == 0 || n < base_size) {
    ...
    auto bits = parlay::delayed::map(
        P, [&](const point& p) { return 1 == ((p.pnt[dims - bit % dims - 1] >> bit / dims) & 1); });
...
auto z_tree(const parlay::sequence<coords>& P, long base_size = node_size_cutoff) {
  ...
  points pts = parlay::tabulate(P.size(), [&](idx i) { return point{i, P[i]}; });
...
void delete_tree(node* T) {  // delete tree in parallel
  if (T->is_leaf)
    ...
    parlay::par_do_if(
        T->size > 1000, [&] { delete_tree(TI->left); }, [&] { delete_tree(TI->right); });
